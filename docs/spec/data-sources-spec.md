...
移してから返す。座標は 1/10000 度・深さは 0.1km・M は 0.1 の整数で格納し、**単位は年ファイル
自身が持つ**（索引にも同じ値があるが、読んだファイル側を信じる方が桁がずれない）。時刻は
「その年の頭からの経過秒」で、起点（`startMs`）も年ファイルが持つ——読む側に日本時間の知識を
要求しないため。
...
- **数値欄の小数部の桁数が年代で変わる。** 1961〜1993 年は 1 桁少なく、緯度分を `474 `
  （= 47.4 分）と書く。空白を無視して `474 / 100` と読むと 4.74 分になり、**緯度が 78km ずれる**。
  1983・1990・1993 年は全件が該当。秒・経度分・深さも同じ規則
...
- 2026-08-25: **長期震源カタログの収録範囲を 1919〜2 日前へ広げた（§6）。** 当初は「確定値が 2023 年までで、同じ粒度で最新まで埋める公開データは無い」と判断して 2009〜2023 年に絞ったが、消費側の機能（日別回数・b 値・静穏化）を具体的に検討した段で気象庁の「震源リスト」（日別 HTML・2024-01-01 以降・2 日前まで）を見つけ、撤回した。あわせて **2 つの限界をデータ自身に持たせた** —— `coveredThroughMs`（終端）と `completeness`（期間ごとの完全性）。前者が無いと「未取得」と「地震が無かった」を描き分けられず、後者が無いと観測網の歴史が地震活動の急減として表示される。収録年を広げる過程で**数値欄の小数部の桁数が年代で変わる**ことが判明した（1961〜1993 年は 1 桁少なく、素朴に読むと緯度が 78km ずれる）。検知の仕組みも作り直し、`console.warn` で終わっていた異常を exit 1 に繋げた。
