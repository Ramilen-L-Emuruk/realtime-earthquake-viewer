...
ため、5 分間隔で警告を出すようにした。**間引きは理由ごとに独立して行う**（1 つにまとめると、
最初に鳴った理由が残りを 5 分間隠して真の原因を取り違える）。

**既知の課題**:
- 較正の落ち着き先がセッションごとに数百 ms ずれる（下記の並走計測で判明。`REG_DELAY_MS` は
  登録遅延の実測 p50 で、実際の遅延は CDN の応答によって変わるため。経緯は §11 の 2026-08-20）

#### 外部時刻サービスとの並走計測（`probeServerTimeOnce`）

較正の主経路を外部の時刻サービスへ置き換えるかを判断するため、同じループでそちらからも時刻を取り、
既存推定器との差分を記録している（`src/services/akamaiClock.ts`）。**この計測はアプリ時計へ
供給しない**（`feedServerSample` を呼ばない）ため、`serverNow()` の挙動は従来どおり。
...
- 失敗したときだけ警告を出し、**棄却理由を併記する**（`network` なら回線の問題、`parse-invalid`
  などならサービス側の形式変更）。理由を落とすと、計測値が出なくなった原因を切り分ける手段が残らない
- ログには最後に較正してからの経過時間（`age`）を併記する。`clock.ts` の K は最後に成功した
  サンプルを保持し続けるため、これが無いと止まった古い較正との差を現行推定器のずれとして
  記録してしまう
...
取得先の選定の経緯と、計測で得られた結論・実測値は §11 の 2026-08-20 の項にまとめている。
主経路を切り替える判断はまだ行っていない（切り替えたらこの節は置き換わる）。
...
- Akamai の時刻サービス（`time.akamai.com`・クロック同期の並走計測）は開発環境から到達可能
...
- `src/services/akamaiClock.ts` — 外部時刻サービスからのサーバー時刻取得（クロック同期の並走計測）
...
- 2026-08-19: 強震モニタの取得を「供給元 → データ時刻順キュー → 反映」の 3 段へ分離した（§4
  ...
  ガードを追加した
- 2026-08-20: クロック同期の主経路を外部の時刻サービスへ替えるかを判断するため、そちらを並走させて
  差分を記録する計測を追加した（§4「外部時刻サービスとの並走計測」）。アプリ時計へは供給しないため
  `serverNow()` の挙動は変えていない。
  ...
  **取得先の選定**: 当初は NICT（情報通信研究機構）の日本標準時 JSON で実装したが、**NICT の
  ...
  アクセス増による負荷が終了理由の一つ）。旧エンドポイント `ntp-a1.nict.go.jp/cgi-bin/json` は
  実際に 404 を返す。JST Clock の表示用ページが使う難読化ホストは今も応答するが、運営者が明示的に
  止めた経路を残存バックエンド経由で叩くことになるため取り下げ、`time.akamai.com` に差し替えた。

  計測で分かったこと（同一環境・同日のブラウザ実測。値そのものより「何に引きずられるか」を見る）:

  | 比較 | 結果 |
  |---|---|
  | 壁時計 − サーバー時刻（応答到着を基準に換算） | RTT と無相関（傾き 0.01 以下） |
  | 壁時計 − サーバー時刻（中点を基準に換算） | RTT に対する傾き -0.5。RTT に比例して誤差が増える |
  | Yahoo 較正 − サーバー時刻 | 落ち着き先がセッションごとに違う（-10ms 台から -500ms 台までを観測） |

  ①**返る時刻は往復の中点ではなく応答生成の時刻**である。中点で換算すると -RTT/2 の系統誤差が丸ごと
  乗る。NICT・Akamai どちらでも同じ傾きが出たため（NICT 側は差し替え前の実装での実測で、コードは
  残していない）、この種のエンドポイント全般に当てはまると考えられる。

  ②**Yahoo 較正の落ち着き先はセッションごとに数百 ms ずれる。** `REG_DELAY_MS` は登録遅延の実測 p50
  で、実際の遅延は CDN の応答によって変わるため、セッション単位で系統的にずれると考えられる。定数の
  調整では消せない性質のばらつきで、standard / DMDSS の両バリアントで同じ傾向を観測した。取得ターゲット
  秒の余裕（`FETCH_OFFSET_MS − (1000 + REG_DELAY_MS) − ずれ`。現状の定数では 1800 − 1480 = 320ms が
  基準）は観測範囲では常に正だったため、主経路を替えても `FETCH_OFFSET_MS` の調整は不要と判断できる。

  あわせて、これまで無音だった「flip を観測できず較正を見送った」経路に警告を追加し（較正が止まっても
  古い K が残って気づけなかった）、較正の鮮度を返す `getServerClockSampleAgeMs` を `clock.ts` に足した。
  この計測とは別の後片付けとして、§4・§5 の「既知の課題」から既に解消済みだった項目（探索窓の固定・
  未較正フォールバックの無警告・Page Visibility 未考慮）を削除した
