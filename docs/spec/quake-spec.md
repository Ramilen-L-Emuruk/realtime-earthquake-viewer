...
3. **発生時刻 ＋ 震源名** — P2PQuake 経路（eventId が配信されないため）
...
> **限界（P2PQuake 経路のみ）**: 同じ分に起きた別々の地震を常に分離できるわけではない。
> - **震源名が同じ場合** — 分離できず 1 枚のカードに統合される
> - **震源が未確定の震度速報が絡む場合** — 震度速報は震源名が空で、同じ分のどちらの地震のものかを
>   電文から判別できない。先に届いた震源付きの報と合流するため、その過程でカードの中身が別の地震に
>   入れ替わって見えることがある（最終的な枚数と表示内容は正しくなる）。なお震度速報も区域別震度
>   （§4 の `isArea: true`）は持っているので、区域の重なりを手がかりにする余地は残っているが、
>   隣接区域の部分一致で逆に誤合流させる危険があるため実装していない
> - **震源要素更新（VXSE61）のカードが残っている間** — この電文は震源名の変更を許す扱いなので、
>   その間は震源名の食い違いを別イベントの根拠にできず、同じ分の別の地震の報を吸収しうる
>
> いずれも P2PQuake の電文に地震を一意に識別する情報が無いことによる。DMDATA 経路は eventId が
> あるため影響を受けない。そもそも同じ分に別々の地震が発表されること自体がまれで、実例として
