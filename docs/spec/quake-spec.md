...
**震度速報の津波区分（`domesticTsunami`）と固定付加文は補わない。** 震度速報はこれらを自前で
持っており（実データでは `調査中` と「今後の情報に注意してください。」）、構造的欠落ではなく
電文が主張する値だから。したがって震源情報が伝えた `なし` と「この地震による津波の心配は
ありません。」は、続報の震度速報が持つ暫定値へ置き換わる。
...
| 消え方 | 例 | 扱い | 補うと何が起きるか |
|---|---|---|---|
| **構造的欠落** | 震度速報は `Earthquake` 要素を持たない（§5） | 前報から**補う** | — |
| **値の取り下げ** | 続報の震度が前報より下がる | **補わない**（そのまま従う） | 気象庁が下げた値を高いまま表示し続ける |

震源が構造的欠落の側だと言える根拠: 気象庁が確定した震源を取り下げるときは訂正報
（`issue.correct`）か VXSE61 を出す。震度速報の続報が「震源が未確定に戻った」を意味することはない。

**取消表示中（`cancelledAt`）のカードからは震源を補わない。** 取消は「その報の内容が誤りだった」
という意味なので、取り下げられた震源を新しいカードへ持ち込むと、気象庁が消した位置を地図に
出し直すことになる。据え置き判定（§6.3）が取消表示中のカードを守らないのと同じ向き。
**同じ危険は震度の補完側に残っている**（あちらはこの判定を持たず、取消済みカードの震度・自由
付加文を引き継ぐ）。震度側も直すなら、根にある「取消表示中のカードが `existing` として選ばれ、
置換で `cancelledAt` ごと消える」ところ——`findExistingQuakeCard` と purge 予約（§6.2）——まで
併せて見直すこと。
