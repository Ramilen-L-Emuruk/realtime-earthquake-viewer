...
タブ切替の要求（`setActiveTabNonRealtime('tsunami')`）は `isNew || upgraded` のときのみ行う。
要求が通れば 15 秒間タブを確保し、その間は地震情報・長周期地震動情報に奪われない。
一方**EEW が確保している間は津波の要求が通らない**（優先順位は
[`audio-tts-spec.md`](audio-tts-spec.md) §6「自動タブ切替の優先順位」）。

続報（同一 eventId の観測点更新等）はタブ切替を要求しない。
`title.showTsunamiTitle()` は続報でも呼ぶのでタイトルバッジは維持される。

**優先度ルール**（tsunami × EEW の同時発報時）:

- tsunami 新規発報・grade 格上げは、EEW が realtime を確保していなければ tsunami タブを表示する。
  **EEW が確保している間は奪えない**（読み上げでも津波は EEW を待つため、声と画面で順序が揃う）。
  EEW のレベル（予報／警報／特別警報）は判定に関与しない
