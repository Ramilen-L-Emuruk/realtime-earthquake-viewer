...
| 無効 | 従来どおり受信の瞬間に `setActiveTabNonRealtime('tsunami')`。**新規発報と grade 格上げだけ**が移し、続報では移らない |

読み上げが有効でも、`setActiveTabNonRealtime('tsunami')` は保険として残っている。**区域を持たない
電文（観測情報のみの続報）が「新規発報」と判定された場合**（進行中の津波の途中でページを読み直すと
直前の状態が分からず、この判定になる）、読み上げ文が組めないため追従が発火しない。この場合だけ
受信時要求へ落として、tsunami タブへ一度も移らない事態を避けている。
