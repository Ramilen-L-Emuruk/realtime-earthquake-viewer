...
| `gates` | **その確信度をどう決めたか**（要求値と中間集計・毎フレーム更新・表示専用）。§8「判定の根拠を見せる」 |
| `confirmedBy` | **確定した瞬間**の判定材料（凍結・未確定は null）。同上 |
...
再読込・不連続リセット（欠測・巻き戻し・5時リロード等）が起きても学習資産は引き継がれる。
**ただし「床の学習が要らない」だけで、立ち上がり判定には助走が要る**（下記「助走」）。
...
結果を出せない理由は 2 つある。`step()` が例外を投げる場合と、**観測点数と震度の件数が食い違って
`step()` へ入れない場合**（同じ `siteConfigId` なのに件数が違う＝上流データの異常）。どちらもログして
次フレームで再試行する（`stateRef` は壊さない）。

**`RESULT_STALL_RESET_FRAMES`(5) フレーム連続で結果を出せなければ検知結果を空にする**——`detections` /
`triggers` / `recentOnsetKeys` に加え、表示用の慢性ノイズ床（`floors` とその対応先 `floorsSites`）も
空にする。学習資産は保持するため、復帰時に学び直しにはならない。前フレームの結果を保持し続けると、
凍結したメンバーを現在の震度に当てて描き続けることになる（理由の詳細は設計書 §23）。

**カウンタは理由ごとに分けない。** 表示から見れば「結果が凍結している」という一点で同じであり、分けると
どちらも閾値に届かないまま交互に起きたときに凍結が解けない。

### 供給が作り直されたとき（リプレイの時間軸の切替）

**データ時刻が空に転じたら、その場で結果を空にする。** 空になるのはリプレイの開始・停止で
`useKyoshinRealtime` が供給を作り直したときだけで、そのとき旧い時間軸の検知は新しい軸に属さない。

上の「結果を出せないフレーム」の仕組みでは足りない。**あちらは `setResult` を呼ばずに抜ける**ので、
結果は前のフレームの値で凍結したまま音・自動タブ切替・地図・検知カード・行動チェックリストへ流れ続ける。
5 フレーム待つ設計でもあり、その間に旧い軸の検知が新しい軸の画面に出る。

**空にするのはレンダー中で、エフェクトまで待たない**（理由と実測は
[settings-pwa-spec.md](settings-pwa-spec.md) §6「強震モニタは自分で落とす」）。

**`stalled` は立てない。** あれは「上流の異常で判らなくなった」ことを表す語で、受け取る側の振る舞いが
変わる（行動チェックリストは判定を保留し、発報側は次の検知を「同じ揺れの続き」として音と通知を省く）。
こちらから作り直した場合にそれをやると、直したい症状がそのまま残る。

学習資産と近傍メタは保持する（時間軸ではなく観測点に紐づくため）。連続失敗のカウンタは 0 に戻す
—— この後に届くのは別の時間軸のフレームで、それまでの失敗の続きではない。

### 助走（供給が始まる前のフレームを先に食わせる）

**供給が始まった時点で既に揺れていると、この検知エンジンはそれを捉えられない。** L1 の立ち上がり
判定（`windowRate`）が見るのは、**エンジン自身が持つ履歴の中での上昇量**だからで、最初のサンプルが
既に高い値だと「ずっと高いまま静止している点」にしか見えない。床（`floorMean`）が学習資産から
正しく復元されていても変わらない —— 判定はしきい値超えではなく上昇量で決まる。

L2 のクラスタ化が入力に取るのは**立ち上がった点だけ**なので、震度4級の単点緩和（`requiredClusterSize`）も
ここでは効かない。あれは「立ち上がった点が 1 つでも成分として認める」緩和であって、立ち上がって
いない点を通す緩和ではない。

実データで測った。大地震 7 本について、地震発生の 1〜12.5 分後を 30 秒刻みで走査した
**164 通りの開始時刻**が母集団。一致の基準は検知の段階（confirmed / likely / なし）と最大震度。

| 開始時点 | 「ずっと再生し続けた場合」と一致 |
|---|---|
| 助走なし | 21 / 164（残り 143 通りは検知が出なかった） |
| 助走 180 秒 | 105 / 164 |
| 助走 300 秒 | 120 / 164 |
| 助走 600 秒 | 156 / 164 |
| 60 秒ブロックで遡り、静穏で打ち切る（採用） | 151 / 164（遡る長さは平均 289 秒） |

**この表がこの測定の単一情報源。** 再現するには `npx tsx scripts/measure-kyoshin-warmup.ts`
（初回は `--fetch` で窓を取得・`--adaptive` で採用した方式・`--warmups=...` で固定長）。
助走の長さを決める定数を変えたら測り直して、この表を更新する。

そこで供給元が、**開始時刻より前のフレームを別に取って検知エンジンだけへ渡す**（経路は
[data-sources-spec.md](data-sources-spec.md) §4「助走」）。どこまで遡るか・打ち切る条件は
`utils/kyoshinWarmup.ts` が単一情報源。

受け取り側の分担は 2 つ。**どのフレームをどの順で食わせるか**は
[`utils/kyoshinWarmupConsumer.ts`](../../src/utils/kyoshinWarmupConsumer.ts) の純関数
（`consumeWarmup` / `pushPendingFrame`）が持ち、`useKyoshinDetectorV2` は
**「待つか・諦めるか」の判断**と、返ってきた事実を記録の文面にすることだけを受け持つ。
React から切り離してあるのは、この処理が可変の状態を 4 つ（検知エンジンの状態・最後に食わせた
時刻・待たせているフレーム・待っているかどうか）同時に動かすため —— エフェクトの中に置くと、
境界の条件（時刻の巻き戻り・観測点集合の版違い・`step()` の例外）を目視でしか確認できない。

守ること。

- **助走が届くまで通常フレームを食わせない**（`pendingRef` に溜めて待つ）。先に食わせると、
  後から届いた助走が「時刻の巻き戻り」として落とされ、助走が丸ごと効かなくなる
- **待つのは時間軸が変わったときだけ**（`waitingWarmupRef`）。初回マウントと、データ時刻が
  空に転じたとき。供給元の識別子（`supplyKey`）の変化で待つと、ローカル履歴アーカイブの
  収録範囲を跨いだだけ（時間軸は続いている）で検知が待ちに入り、結果が古い値で凍結する
- **待たせたフレームには観測点集合の識別子を持たせ、食わせる直前に照合する**。待っている間に
  観測点リストの版が替わりうる。長さの検証だけでは、点数がたまたま一致したときに通ってしまう
- **同じデータ時刻を二重に積まない**。助走は通常フレームと別の経路で届くので、データ時刻が
  変わらないまま助走だけが届くレンダーが必ず起きる。二重に積むと、待たせていた最後のフレームの
  検知結果が画面へ出ない（次のフレームまで音も自動タブ切替も動かない）
- **届かないまま `WARMUP_WAIT_MAX_FRAMES` に達したら諦めて動き出す**。供給元は必ず 1 度
  `prefill` を呼ぶ契約だが、破られたときに検知が恒久的に沈黙しないための安全弁
- **届いたのに 1 件も使えなかったら記録する**（版違いで全部落ちた場合）。画面からは「助走が
  無かった」のと見分けが付かない
- **消化中に例外が出ても外へ出さない。握りは `step()` の呼び出しに限らない。** このフックは
  `App()` の中で呼ばれるので、投げるとエフェクトごと未捕捉例外で抜け、**根のエラー境界まで
  飛んで画面全体が落ちる**（地図も地震カードも設定も消える）。握りは 2 段 —— 1 フレームぶんの
  消化を丸ごと囲って**残りは続ける**（件数を数えてまとめて 1 行に残す。助走は最大 900 件
  あるので 1 件ずつ出すとログが埋まる）、その外側で助走・待機分それぞれの列を囲って、
  **反復そのものが壊れたらその段だけ打ち切る**（そこまでに食わせた分は残る）
...
### 判定の根拠を見せる（`utils/detectionGates.ts`）

確信度のチップが「どの段か」を示すのに対し、こちらは「**なぜその段なのか**」を示す。
文言の組み立ては `detectionGates.ts` が単一情報源（`gateShortfall` / `gateRows` / `gateNotes`）。

**割合や点数のような数値の確信度は作らない。** エンジンは確率モデルではなく決定的なゲートの通過判定で
段を決めており（§4 L4）、中間の連続量を持っていない。出せるのは「どの条件をいくつ満たしたか」と
「あと何が足りないか」だけで、それを割合へ均すと実装が持っていない精度を装うことになる
（経緯は設計書§34）。

| 出す場所 | 中身 |
|---|---|
| 検知カードの一行（折りたたみを開かなくても見える） | 確定したものは**確定した瞬間の**内訳で「6点・震度2で確定」。まだのものは「確定まで あと2点」。言うことが無ければ行ごと出さない |
| 検知カードの折りたたみ（既定は閉じる） | 地域ごとのゲート表と、要求値が動いた理由 |
| 診断ログ | 記録を開いた瞬間の内訳と、確定した瞬間の内訳（[`settings-pwa-spec.md`](settings-pwa-spec.md) §12） |

守ること。

- **`gates` を判定の入力にしない。** 分岐で読み返すと、表示のための構造体が判定の一部になる。
  書くのは `updateEventMetrics` が確信度を決め終えた後の 1 箇所だけ
- **イベント自身が既に持っている値（`lastSize`・`maxIntensity`・`confirmStreak`・`everNeighborRise`）を
  `gates` へ複製しない。** 複製すると、併合（`mergeAdjacentEvents`）が本体だけを書き換えたときに
  二重管理が食い違う
- **確定したものの根拠は `confirmedBy`（凍結）から採る。** 現フレームの `gates` で言い直すと、
  減衰した後に「確定しているのに条件をどれも満たしていない」表になる
- **併合では `confirmedBy` を確定時刻の早いほうから採り、`intenseCount` はバーが揃うときだけ合算する。**
  `intenseCount` は慢性活性セルかどうかで変わる `confirmIntensityReq` というイベントごとに違うバーで
  数えた値なので、単純に足せない。バーが食い違うときは数え直せない側を混ぜず、より厳しいバーで
  実際に数えた側の値だけを残す（欠けた数を出すほうが、違うバーで数えた点を混ぜた数を出すよりまし）
- **「秒読み中か」を確信度より先に見る。** 確定の条件は周囲の裏付け（`everNeighborRise`）を見ないが
  `likely` へ上げる条件は見るため、「確定へ秒読み中なのに確信度は faint」が起こる。確信度で先に
  分岐すると、確定には要らない条件を残り条件として示す
- **表の「揺れ継続中の点」はカード本文の「N観測点で反応」と数え方が違う。** 前者は `lastSize`
  （床を明確に超えて継続中 or 直近に立ち上がった点）、後者は現在震度0以上の点。同じ画面に 2 つの
  点数が並ぶので、表の下に基準が違う旨を出す
- **折りたたみの地域番号はイベント ID の順で振る。** `step()` が返す並びは最大震度の降順で毎フレーム
  並べ替わるため、渡された順に振ると震度が近い 2 地域の間でラベルだけが入れ替わる
- **一行に出す代表は、最上位の確信度のうち最大震度が最大の 1 件。** `deriveKyoshinView` が主 likely を
  選ぶのと同じ基準（同§「表示状態への変換」）。地域ごとの内訳は折りたたみへ回すので、一行は常に 1 件
  だけを指す。**折りたたみの並び（イベント ID 順）とは基準が違う**ため、複数地域あるときの一行が
  「地域1」を指すとは限らない
- **見出しに検知エンジンの内部用語を出さない。** 「フレーム」（1 秒ごとの処理単位）・「spread」・
  「近傍一致」といった語は仕様書にしか定義が無く、表を開いた利用者には手がかりが無い。連続して条件を
  満たした回数は「あと約N秒」と時間で言い、判定が 1 秒ごとであることは表の下の注記で伝える
...
**保持値を捨てる条件は 2 種類あり、順番に意味がある。**

| 種類 | 条件 | いつ効くか |
|---|---|---|
| 位置の対応が壊れた | 観測点集合の識別子が変わった／件数が変わった | **データ時刻を見ない。時刻のガードより前に判定する** |
| 時間軸が変わった | データ時刻が巻き戻った | 時刻が読めるフレームでのみ |

前者を時刻のガードより前に置くのは、**供給を作り直したとき（リプレイの時間軸の切替）が「件数 0・
データ時刻なし」の形で届く**ため。後ろに置くと素通しして、旧い時間軸の保持値が新しい軸の最初の
フレームで蘇る（同じ観測点集合・同じ件数で、時刻が前へ進んでいれば、後者の条件にも掛からない）。
位置の対応が壊れたことは時刻を見なくても判るので、順番を入れ替えても他の判定には影響しない。
...
| confirmed 立ち上がり（初検知） | realtime タブ＋タイトル「揺れ検知」＋検知音＋ブラウザ通知。ただし**「判らなくなって」消えた直後の復帰では鳴らさない**（下記） |
...
**「判らなくなった」ことによる消失と、揺れが収まったことによる消失を見分ける。** 検知結果を出せない
フレームが続くと結果は空になり（前節）、`confirmed` が落ちる。しかしこれは揺れが終わったのではなく、
検知そのものが止まっただけ。復帰時に素朴なエッジ検出で再発報すると、**同じ地震で検知音とブラウザ通知が
鳴り直し**「収まって、また始まった」と受け取られる。

見分けは検知エンジンが返す `stalled`（結果を出せずに空にしたか）で行う。`stalled` で消えたなら印を立て、
そのとき確定していた検知地点を控えておく。**結果が戻った最初のフレームで検知が立ち、かつその地点が
すべて途絶前の近くにある**なら同じ地震の続きとみなし、音と通知だけ抑える（タブとタイトルは戻す。
判っていない間まで「揺れ検知」を出し続けない）。

**場所の照合は省けない。** 途絶が `MAX_DT_GAP_MS` 以内なら検知エンジンは状態を組み直さず通常の経路を
通るため、復帰したフレームで**まったく別の地震**が確定しうる（EEW 発表中や高震度は 1 フレームで確定する
設計なので、強い地震ほど起こりやすい）。物差しは `MERGE_EVENT_KM`——イベント重心がこの距離以内なら
1 地震として併合する、という検知エンジン側の判断と揃える（照合しているのがまさに重心どうしのため）。
**1 つでも離れた地点が混じっていれば鳴らす側へ倒す**——余分に鳴るのは煩いだけだが、抑えすぎると
無関係な地震が無音で消える。

**経過時間では測らない。** 起点も時計も揃わず、窓を閉じ忘れれば同じ「無音で握り潰す」失敗になる。

**揺れが収まって消えた場合は印を立てない**（直後に別の地震が来たとき鳴らなくなる）。判断の経緯と、
時刻で測ろうとして破綻した内訳は[設計書§35](kyoshin-detection-v3-design.md)。

**揺れの強まり（レベルアップ・再エスカレーション）と別地点の検知では、地図もその点へ短く寄る**（揺れフォーカス）。確定検知の開始は対象外——そちらは検知点全体へのフィットが画を作る。寄り先は**最大震度を記録した観測点**
（`ConfirmedShock.peak`）で、地域の照合に使うメンバー重心とは別に持つ。要求は音を鳴らすのと同じ
タイミングで出す（レベルアップ・再エスカレーションの分岐をそのまま使う）。**ただし寄り先
（`focusPointRef`）は confirmed でなければ null**——likely（候補音）中は要求が出ても画は動かない。
控えめな音量で知らせるだけに留め、確信度に見合わない大きさで画を動かさないため（候補音の音量を
抑えているのと同じ考え方）。要求を出した後の寄せ方・寄せない条件は
[`map-rendering-spec.md`](map-rendering-spec.md) §6「揺れフォーカス」が単一情報源。
...
1. **アクティブな EEW（震源要素確定済み）ごとの個別判定**: 各 EEW について、震源・発生時刻から
   S波の地表到達半径（予報円と同じ JMA2001 走時表。→ `utils/travelTime.ts`）×安全マージン係数
...
**床は観測点と同じ並びの配列で受け渡す。座標から作ったキーで引かないこと。** 同一座標に複数の実観測点が
載るため（`computeSiteKeys` が出現順に `#2`, `#3` と別実体へ分ける。全1725点中207グループ・431点）、座標を
キーにすると 2 つ目以降がグループ先頭の**別センサーの床**で判定される。実データ 15 分では 60 点の表示が
食い違い、多くは「床の高い都市部の点が低い床で判定されて素通りする」向きだった。キーの生成規則を
2 箇所に持たせず、並びで対応づける。

位置で対応づける以上、**床がその観測点配列に対して計算されたものかを呼び出し側で確かめる**
（`KyoshinDetectorV2Result.floorsSites` との参照比較）。床が作り直されるのは**次にデータ時刻が進んだとき**
なので、観測点リストが差し替わってからそれまでの間（途中に別のレンダーを挟むこともある）は「新しい観測点 ×
古い床」が並ぶ。点数が偶然一致すると長さの検査では捕まらない。差し替えは `siteConfigId` の更新だけでなく、
過去日のリプレイ開始でも起こる。
`step()` の継続失敗で検知結果がリセットされたときも床は空になり、同じく不一致として扱われる（§7）。

照合が取れないとき、および**渡す配列の長さが揃わないとき**（`filterSubThresholdIndices` 自身の検査）は、
フィルタを通さず、欠測を保持したままの表示値をそのまま描く（消すより出す方へ倒す。保持値と生値の区別は
§8）。後者が効くのは観測点数と震度の件数が食い違うときで、このとき観測点配列は差し替わっていないため
**参照の照合は通り、長さだけが合わない**。

素通りが続くなら、主な原因は記録に残る。観測点リストの取得が失敗し続ける場合と、`step()` が壊れ続ける
場合、観測点数と震度の件数が食い違う場合（後の 2 つは [data-sources-spec.md](data-sources-spec.md)
§4「取得と再生の分離」）。**記録に現れない経路も 1 つ残っている**——観測点リストを並行して取りに行った
ときの応答順の入れ替わりは、取得自体は成功しているので失敗の記録には出ない（同 §4「残る既知の課題」）。
