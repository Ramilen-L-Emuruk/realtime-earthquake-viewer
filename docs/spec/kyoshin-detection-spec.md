...
`useKyoshinDetectorV2` 側の受け取りで守ること。
...
- **届いたのに 1 件も使えなかったら記録する**（版違いで全部落ちた場合）。画面からは「助走が
  無かった」のと見分けが付かない
- **消化中に `step()` が投げても外へ出さない。** このフックは `App()` の中で呼ばれるので、
  投げるとエフェクトごと未捕捉例外で抜け、**根のエラー境界まで飛んで画面全体が落ちる**
  （地図も地震カードも設定も消える）。1 件の失敗で残りの消化を止めず、件数だけ数えて
  まとめて 1 行に残す（助走は最大 900 件あるので 1 件ずつ出すとログが埋まる）
