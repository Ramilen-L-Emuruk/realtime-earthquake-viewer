...
1. **confirmed 判定**: 次の (a) (b) のいずれかが `confirmFramesReq` 連続で成立したら `everConfirmed=true` の
   ...
   **ただし単点のまま居座る確定は降ろす（設計書§33）。** `everConfirmed` が立っていても、初めて確定した
   時刻から `SOLO_CONFIRM_GRACE_MS` を過ぎて **なお一度も「隣り合う 2 点」になっていなければ**、
   `confirmed` として扱わない。上限に張り付いた観測点は永久に単点のまま生き続けるため、これが無いと
   降りる契機が存在しない（イベント自体は `lastSize > 0` の間 `HOLD_MS` が満了しない）。
   ...
   - 判定は `isSoloConfirmStale` に集約する。確信度を書く箇所は `updateEventMetrics` と
     `mergeAdjacentEvents` の 2 つあり、片方だけに置くと併合が走ったフレームだけ 1 フレーム復活する
   - **降ろした先は `weak` に固定する。** 併合で `lastSize` は合算されるため、離れた 3 点でも
     `MIN_LIKELY_POINTS` に届いて `faint`・`likely` へ回り込める（`likely` は候補音を鳴らす）
   - 遅れて隣接する点が続けば自力で `confirmed` へ戻る
...
**揺れの強まり（レベルアップ・再エスカレーション）と別地点の検知では、地図もその点へ短く寄る**（揺れフォーカス）。確定検知の開始は対象外——そちらは検知点全体へのフィットが画を作る。寄り先は**最大震度を記録した観測点**
（`ConfirmedShock.peak`）で、地域の照合に使うメンバー重心とは別に持つ。要求はこのフックが音と同じ判定で
出し、寄せ方・寄せない条件は [`map-rendering-spec.md`](map-rendering-spec.md) §6「揺れフォーカス」が
単一情報源。**likely（候補音）では寄せない**——控えめな音量で知らせるだけに留め、確信度に見合わない
大きさで画を動かさないため（候補音の音量を抑えているのと同じ考え方）。

同じフレームで「揺れの強まり」と「別地点の検知」が両方成立したら、**寄り先は別地点を採る**（音は
両方鳴る）。別地点は「別の場所でも揺れている」ことをフル音量で知らせる報せで、同じ地震の強まりより
見せる価値が高いため。
