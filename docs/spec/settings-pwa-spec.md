...
`testData.ts` が抱える能登本震の実データ 3 つ（`src/data/noto-honshin-2024-*.json`）は合わせて
824 KB ある。静的に取り込むと**テストボタンを一度も押さない利用者にも初回表示で届く**ので、
...
**市町村の未入電（`City/Condition`）はこのデータに入っていない。** 気象庁の解説資料が定める発表条件
（配下に未入電の観測点があり、かつその市町村の最大震度が震度4以下（又は入電なし））に、能登本震の確定報が当たらない
ため。実電文を 3 日分（能登本震・豊後水道・日向灘の計 115 通）走査しても 1 通も見つからなかった。
**手で足さない** —— 実際には起きない形をテストデータに置くと、そちらへ合わせた実装が入りうる
（→ [`quake-spec.md`](quake-spec.md) §5「市町村の震度」）。
