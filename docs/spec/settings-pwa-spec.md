...
| 訓練報テスト（DMDSS 版のみ） | `createTestEarthquake(true, '訓練')` | - | 中身は地震テストと同じで、**電文ヘッダの運用種別（`Control/Status`）だけが違う**。本物と同じく画面・音・読み上げへ流し、カードに「訓練報」の印を出す （→ [`quake-spec.md`](quake-spec.md) §5「電文の運用種別」）。**この印を出す手段がこれしかない** —— 実配信の訓練報は事前に予告されて流れるもので、こちらの都合では受け取れない |
