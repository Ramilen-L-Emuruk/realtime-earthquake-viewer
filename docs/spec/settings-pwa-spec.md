...
- データはDMDATA由来の実電文ではなく、気象庁公表資料を典拠に手作業で書き起こしたもの
  （`ReplayPayload`をそのまま記述する。`capture-test-scenario.ts`のような実電文からの
  自動キャプチャではない）。そのため `public/data/test-scenarios/*.json` と異なり
  `.gitignore` の対象外にしている。DMDATA.JP利用規約が制限するEEWの二次配信はDMDATA由来の
  データに適用されるものであり、気象庁公表資料からの独自の書き起こしには当たらないという
  開発者判断による（法務確認は経ていない）
- 手作業で書き起こす性質上typoが混入しやすいため、震度値（`IntensityScale`）や津波の警報
  区分など描画に直結する値は
...
- 現時点で収録しているのは2011年東北地方太平洋沖地震のみ。本震のEEW全15報（気象庁の
  発表状況詳細ページの実測値。ただし地域は宮城県・岩手県・福島県・茨城県に絞り、周辺県は
  省略）・警報級の誘発地震19件（2011/3/11 17:40〜3/13 10:26。各地震1報の代表値のみで、
  震源座標は概算・全報の推移は追っていない）・地震情報のマグニチュード改定4報
  （M7.9→8.4→8.8→9.0）・大津波警報の発表から解除までの5段階（発表→10m以上へ引き上げ→
  津波警報へ切替→津波注意報へ切替→全解除。各段階で複数地域・複数区分（大津波警報／津波
  警報／津波注意報）を同時収録しているが、気象庁発表が対象とする全国70以上の予報区の
  一部に絞っている）を収録している
