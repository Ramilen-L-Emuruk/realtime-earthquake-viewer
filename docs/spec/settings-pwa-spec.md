...
指定すると、その取得元にはそもそもデータが無い。2011年東北地方太平洋沖地震のような
過去の大地震を再現できるよう、アプリに同梱したローカルJSON
（`public/data/historical-archives/`）で代替する仕組みを用意している。

- 収録範囲の一覧は `index.json`（`id`・`label`・`description`・収録範囲`from`/`to`）。
  本体は `{id}.json`。設定タブの「テスト時刻設定」に収録済みアーカイブとして表示される
...
- データはDMDATA由来の実電文ではなく、気象庁公表資料を典拠にしたもの（`ReplayPayload`を
  そのまま記述する。`capture-test-scenario.ts`のような実電文からの自動キャプチャではない）。
  そのため `public/data/test-scenarios/*.json` と異なり `.gitignore` の対象外にしている。
  DMDATA.JP利用規約が制限するEEWの二次配信はDMDATA由来のデータに適用されるものであり、
  気象庁公表資料からの独自の変換には当たらないという開発者判断による（法務確認は経ていない）
- EEW（緊急地震速報）と津波警報・注意報は、気象庁が公開する発表状況ページを
  [`build-historical-archive-2011-tohoku.ts`](../../scripts/build-historical-archive-2011-tohoku.ts)
  が機械的に取得・パースして生成する（`npx tsx scripts/build-historical-archive-2011-tohoku.ts`
  で再生成できる）。手作業の書き起こしではなく、パーサー
  ([`historicalEewParser.ts`](../../scripts/historicalEewParser.ts)・
  [`historicalTsunamiParser.ts`](../../scripts/historicalTsunamiParser.ts)) が生HTMLの表を
  直接読む。地震情報のマグニチュード改定4報のみ、対応する機械可読な一覧ページが無いため
  手作業の記録を維持している
...
- 現時点で収録しているのは2011年東北地方太平洋沖地震のみ。本震のEEW全15報＋警報級の
  誘発地震19件（2011/3/11 17:40〜3/13 10:26）の全報を、地域・震度とも全国分（気象庁の
  発表状況詳細ページの実測値）収録している。震源座標は各報自身が持つ暫定値（震源要素が
  報ごとに変わる、実際の運用と同じ挙動）。地震情報のマグニチュード改定4報
  （M7.9→8.4→8.8→9.0）・大津波警報の発表から解除までの全12段階（3/11 14:49〜3/13 17:58。
  気象庁発表が対象とする全国66予報区のうち、各段階で実際に警報・注意報が出ていた地域を
  全て収録）を収録している
  - **津波の予測高さが欠けている地域がある**: 気象庁の検証ページが公開しているのは
    ...
    [`historicalTsunamiArchiveBuilder.ts`](../../scripts/historicalTsunamiArchiveBuilder.ts)
    の判定ロジック参照）。岩手県・宮城県・福島県・青森県太平洋沿岸・茨城県・
