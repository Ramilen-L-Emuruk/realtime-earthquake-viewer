...
| EEW 警報テスト | `createTestEEWWarning(withDmdssFields)` | 50 | 震度 5 強相当・警報（日向灘 M6.5）。震度 4 の区域を 1 つ含み、同一電文内の予報域も確認できる。**最大予測値の変化（`Appendix`）は実電文の形で入れてある** —— 第 1 報は要素ごと持たず、2 報目で「大きくなった」を立て、3 報目以降は値が 0 に戻る。3 回押せば、値が戻ったあとも帯が残ること（消えるのは 2 回目を押してから 10 秒後）まで実機で見られる（→ [`eew-spec.md`](eew-spec.md) §3「帯を出し続ける長さは、報の状態と切り離してある」）。**毎報立てる形にしないこと** —— 帯が出続けるので、保持が効いているのか電文が毎報言っているだけなのかを画面から見分けられない |
