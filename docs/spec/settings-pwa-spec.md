...
| 地震テスト | `createTestEarthquake(useDmdataShape)` | - | 令和 6 年能登半島地震の実データベース（`src/data/noto-honshin-2024-*.json`）を採用。`points` の形と情報種別はバリアントで切り替える（後述「実電文の形に合わせる」）。**DMDSS 版では長周期地震動観測情報（VXSE62）も併せて出す** —— 観測情報の種類を 4（全国の最大階級が 3 以上で、階級 3 以上を観測した地域のうち最大震度が 4 以下の地域がある）にしてあり、高層階が大きく揺れたことを伝える一言が出る経路を実機で通せる（→ [`quake-spec.md`](quake-spec.md) §8） |
