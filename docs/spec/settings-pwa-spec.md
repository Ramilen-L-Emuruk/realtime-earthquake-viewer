...
| EEW 誤報取消テスト | `createTestEEWWarning(withDmdssFields)` + `EEW_RETRACTION_CANCEL_MS`(10s) 後に取消 | 50→取消 | 10 秒後に `cancelled:true` 電文で `eewCancel` 音・通知・読み上げを検証。**DMDSS では取消電文に取消しの概要（`Body/Text`）を持たせる** —— 気象庁が書いた取り消しの理由で、アプリの定型文とは別物（→ [`quake-spec.md`](quake-spec.md) §8「取消しの理由は電文にしかない」） |
...
| 津波誤報取消テスト | `createTestTsunamiRetraction(withDmdssFields)` + 90 秒後に取消電文 | - | 警報・注意報混在の発表 → 90 秒後に電文全体が取り消される（DMDSS は `retracted` 経路。standard は理由を判別できないため「解除」表示）。**DMDSS では取消電文に取消しの概要（`Body/Text`）を持たせる** —— 気象庁が書いた取り消しの理由で、アプリの定型文とは別物（→ [`tsunami-spec.md`](tsunami-spec.md) §6） |
