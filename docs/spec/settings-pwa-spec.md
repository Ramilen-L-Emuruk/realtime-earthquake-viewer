...
| セクション | 主な項目 |
|---|---|
| DM-D.S.S 接続設定（DMDSS 版のみ） | 接続状態（再生中の表示は → §6「再生中は DMDATA の WebSocket を止める（接続状態の表示）」）・API キー（BYOK 方式）・試験報の受信 |
...
#### 再生中は DMDATA の WebSocket を止める（接続状態の表示）

**取得の成否に関わらず、DMDSS 版で再生中は DMDATA の WebSocket を切っている**（アーカイブ再生と
ライブ受信が混ざるため）。このとき §2 の「接続状態」は専用の表示**「再生中（ライブ受信は停止）」**になる。

`ConnectionStatus` に `replay` を設けているのはこのため。`disconnected` と混ぜると地図に切断警告が出て
意図した停止が異常に見え、逆に更新しないままだと直前の値（多くは「接続中」）が残って実態と食い違う。
standard 版は再生中も P2PQuake のライブ受信を続けるため対象外（この間も接続状態は実態を示す）。
...
- 2026-08-19: DMDSS 版のテスト時刻設定で、DMDATA アーカイブの取得に失敗しても強震モニタの再生を
  ...
  DMDATA の WebSocket を切っているのに `connectionStatus` を更新しておらず、直前の値（多くは「接続中」）が
  残って実態と食い違っていた。取得の成否に関わらず再生中は常に該当する既存の不整合だが、上記の変更で
  失敗時も再生が続くようになり食い違う時間が延びるため、あわせて直した
