...
| 地震テスト | `createTestEarthquake(useDmdataShape)` | - | 令和 6 年能登半島地震の実データベース（`src/data/noto-honshin-2024-*.json`）を採用。`points` の形と情報種別はバリアントで切り替える（後述「実電文の形に合わせる」）。**DMDSS 版のデータは実電文をパーサーへ通して作る**（`npm run build-test-quake`）。手で組み立てていた頃は観測点が市町村に紐付いておらず、震度一覧の 4 段目（県 → 一次細分区域 → 市町村 → 観測点）を実機で一度も確かめられなかった。**「震度5弱以上未入電」も実電文のまま 3 地点入っている**ので差し替えを入れていない（standard 版だけは、元資材が観測点しか持たず未入電を含まないため 2 地点を差し替える）。**DMDSS 版では長周期地震動観測情報（VXSE62）も併せて出す** —— 観測情報の種類を 4（全国の最大階級が 3 以上で、階級 3 以上を観測した地域のうち最大震度が 4 以下の地域がある）にしてあり、高層階が大きく揺れたことを伝える一言が出る経路を実機で通せる（→ [`quake-spec.md`](quake-spec.md) §8）。**固定付加文（その他）も DMDSS 版だけが持つ**（P2PQuake は付加文を配信しない）。元電文の 2 文のうち `＊`（気象庁以外が運用する観測点。→ [`quake-spec.md`](quake-spec.md) §8「気象庁以外が運用する観測点（`＊`）」）を説明する一文だけを渡し、もう一方の「震源要素を訂正します。」は落とす —— このデータは訂正報ではなく確定報の形（`issue.correct: 'なし'`）なので、残すと訂正していない報が訂正を名乗る（→ [`quake-spec.md`](quake-spec.md) §8「固定付加文（その他）（`VarComment/Text`）はそのまま出す」） |
