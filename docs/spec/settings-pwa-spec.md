...
| EEW 単独点処理テスト | `createTestEEWAssumed()` | 初報 なし → 続報 50 | 震度予想なし・予報（`condition: '仮定震源要素'` かつ区域なし＝ PLUM 1 点相当の報。日向灘・気象庁が入れる固定値 M1.0 深さ 10km）。続報で震源が確定し震度 5 強予想の警報へ格上げされる（再クリックしなくても最終報で同じ格上げが起きる。後述「実電文の形に合わせる」）。「単独点処理のため、予想震度なし。」を待たずに読み、続報で言い直す経路を確かめる（[`audio-tts-spec.md`](audio-tts-spec.md) §6）。**格上げの伝え方は再クリックのタイミングで変わる**ため、両方の経路をこのボタンで確かめられる（震源を読み終えてから押せば「緊急地震速報に切り替わりました。」／読み上げ中に押せば割り込んで言い直し。同 §6「予報から警報へ上がったとき」）。**震源名は報をまたいで変えない**（変えると「震源を更新」の経路に入る） |
| EEW 深発地震テスト | `createTestEEWDeep()` | なし | 震度予想なし・予報（深発地震・深さ 450km。小笠原諸島西方沖 M6.5）。区域なし。「深発地震のため、予想震度なし。」を待たずに読む経路を確かめる。続報も予報級のまま（理由は後述「実電文の形に合わせる」）。**震源要素の精度は EPOS（rank 7・8）を持たせてある** —— 警報のテストが IPF 法（rank 3）なので、分けないと「EPOS（海域〔観測網外〕）」の括弧書きがどのテストボタンでも画面に出ない |
| EEW 誤報取消テスト | `createTestEEWWarning()` + `EEW_RETRACTION_CANCEL_MS`(10s) 後に取消 | 50→取消 | 10 秒後に `cancelled:true` 電文で `eewCancel` 音・通知・読み上げを検証。**DMDSS では取消電文に取消しの概要（`Body/Text`）を持たせる** —— 気象庁が書いた取り消しの理由で、アプリの定型文とは別物（→ [`quake-spec.md`](quake-spec.md) §8「取消しの理由は電文にしかない」） |
