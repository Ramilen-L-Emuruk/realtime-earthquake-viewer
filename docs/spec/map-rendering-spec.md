...
- **第一報** → その EEW 自身の S 波円（円が無ければ震源 1 点）へ寄る。予想の区域塗りは含めない——
  ...
  - この寄りを見せた直後の **3 秒間**（`CameraFollowsGL.tsx` の `GROWTH_FOLLOW_SUPPRESS_MS`）は、
    下記「発報中」の**引き直しだけ**を止める。すでに揺れ検知が広範囲に出ている状況では、寄った直後に
...
購読し、ユーザーが地図を操作している間は自動フィットを抑制する。抑制は一定時間（`gl/camera.ts` の
`INTERACTION_HOLD_SEC`）で**必ず解ける**。

この時間は設定タブの「自動復帰までの時間」とは**切り離している**（あちらはタブの自動復帰専用）。
以前は同じ設定値を使っており、設定次第で抑制が永久に解けなくなっていた（経緯は §15 の 2026-08-19）。
...
- 2026-08-18: EEW 追従で第一報の直後 3 秒は引き直しを止める挙動を §6 に明記（**ドキュメントのみ**。
  ...
  ない」と誤解される元になっていた
- 2026-08-19: 地図の自動追従の抑制を、設定「自動復帰までの時間」から切り離して固定時間にした（§6。
  08-18 の誤認識の修正とは**別の原因**）。同じ設定値を共有していたため、「無効」を選ぶと解除タイマーが
  張られず、地図を一度触るだけで揺れ検知・津波の追従が二度と戻らなかった。当時フラグを下ろせたのは
  地震カードの明示クリックと新規 EEW だけで、その経路を持たない追従には復帰の術が無かった。設定の
  意図（タブを勝手に切り替えさせない）と副作用（地図も動かなくなる）が食い違ううえ、追従ごとに解除を
  足す方針では漏れが避けられないため、結合そのものを断った
