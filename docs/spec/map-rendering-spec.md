...
- **発報中** → 「S 波円 ∪ 震源 ∪ 揺れ検知点 ∪ 予想の区域塗り」を 1 つに束ねた範囲。表示に収まらなく
  なったときだけ引く（**第一報の直後 3 秒間は上記のとおり引かない**）。EEW の件数が減ったときの
  再フィットも同じ範囲を使う（目標が食い違うと、再フィットの直後に引き直しが起きて二段のカメラ移動に
  なる）
  - ただし**件数減少時の再フィットだけは第一報直後の抑制を見ず、表示に収まっているかどうかも見ない**。
    条件（EEW 数か波円数の減少）を満たせば抑制中でも寄り直す。複数の EEW が同時に出ている場面で、
    片方が取り消されたときなどに、残りを必ず画に収めるための経路
...
購読し、ユーザーが地図を操作している間は自動フィットを抑制する。MapLibre はプログラムによる
...
ユーザー操作と誤認していた。結果、誰も地図に触れていないのに EEW のカメラ追従が「自動復帰までの時間」
（既定 30 秒）ぶん止まり、予想の区域塗りが画面外に取り残されていた。
...
- 2026-08-18: EEW 追従で第一報の直後 3 秒は引き直しを止める挙動を §6 に明記（**ドキュメントのみ**。
  ...
  ない」と誤解される元になっていた。あわせて、この抑制が及ばない経路（件数減少時の再フィット）も明記
