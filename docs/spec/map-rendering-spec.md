...
- 画からはみ出したら引く（**成長フォロー**）。収まっている場合は、寄り直して **1 段以上**
  ズームが深くなる状態が **8 秒**続いたら寄り直す（**縮小フォロー**。値は `CameraFollowsGL.tsx` の `SHRINK_*`）
  - 判定に「画が目標の何倍ゆるいか」という**矩形の広さの比は使わない**。比で測ると padding（px 固定）と
    ビューポートのアスペクト比が効き、地図ペインの短辺が 300px を割る状態（スマホの上下分割など）では
    着地後も閾値を超えたままになって無駄な寄り直しを繰り返す。ズームの利得で測れば着地後の利得は
    必ず 0 になるため、その往復が構造的に起きない（`gl/camera.ts` の `zoomGainForBounds`）
  - 8 秒という保持時間は、余震・表面波の再来・欠測の出入りで範囲が数秒単位で上下するため。
    一瞬狭まっただけで寄せるとカメラが落ち着かない。EEW 発報中・ユーザー操作中・自動フィットの
  ...
  - 縮小フォローを持つのは揺れ検知の追従だけ。EEW 発報中は下記の EEW 側が目標を持つ
...
- 2026-08-20: 揺れ検知の追従に**縮小フォロー**を足し、追う対象を「地図に描かれている検知点」へ揃えた（§6）。
