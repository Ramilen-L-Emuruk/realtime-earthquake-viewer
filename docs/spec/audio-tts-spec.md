...
- 全種別が単一セッションを共有する。**EEW については対処済み**で、非 EEW の読み上げは EEW の
  読み上げが途切れるのを待つ（§6「EEW の読み上げは 1 本に直列化する」）。一方**非 EEW 同士には
  優先度が無く、いまも互いを打ち切る**。震度速報の更新が古い震度速報を消すのは意図した動作だが、
  大津波警報の読み上げが後続の地震情報に消されるケースが残っている（2024/1/1 能登の再生で確認）
...
**非 EEW の読み上げ（地震情報・津波・長周期・南海トラフ）は、EEW の読み上げが途切れるのを待つ**
（`NON_EEW_SPEECH_MAX_WAIT_MS` = 20 秒を上限に、超えたら諦めて読む）。待たせないと緊急度の低い
情報が EEW の発話を消す。実例として、2024/1/1 能登の 16:08 の EEW 第 1 報が、その 0.36 秒後に
**読み上げの始まった**震源情報に潰されていた（震源情報の電文自体は EEW より先に届いており、
上の表の遅延を経て読み上げが始まる。**割り込みは電文の到来順では決まらない**）。

**逆向き（EEW が地震情報の読み上げを切る）は許す。** 緊急度どおりであり、また地震情報の本文は
数千文字に達することがあって（能登本震の地震情報は読み切りに 2 分近くかかる）、その後ろに EEW を
並べると致命的に遅れる。
