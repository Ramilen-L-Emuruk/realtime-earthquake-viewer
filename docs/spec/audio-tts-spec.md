...
第 1 フェーズの発話完了を待ってから話すため、両者は重ならない。EEW の音を作り変えても、
ここに調整すべき遅延値は無い。
...
| 続報で震度・階級が上がった | 差分だけを短く「震度5強に引き上げ。」。2 秒（`EEW_UPGRADE_DEBOUNCE_MS`）静まってから最新値を 1 回 |
| 続報でレベルが上がった | 区分（「警報に切り替わりました。」／「特別警報に切り替わりました。」）を前置きし、待たずに予想値を読み直す。震度が未確定なら「予想震度なし。」が続く |
| 続報で下がった | 読まない |

**待つ対象は時間ではなく値の確定。** 時間で待つと、続報が立て続けに届く大地震ほど読み上げが
遅れるため（経緯は §10 参照）。

**レベルの格上げは震度の引き上げより優先する。** 震度が伸びて特別警報の条件（震度 6 弱以上）を
跨ぐと両方が同時に上がるが、その場合も差分の短句ではなく区分を前置きしたフル文言を読む。
値の変化だけを追うと、もっとも重い「特別警報に切り替わった」ことが声から抜け落ちるため。
