...
- `POST /mora_length` — 繋ぎ合わせたアクセント句の音素長を引き直す（下記「辞書」節で使う）
...
該当した語は前後と切り離して合成するため継ぎ目の抑揚が不連続になりがちで、これを目立たせない
よう地名の直後には短い間（`DICT_TRAILING_PAUSE` = 0.12 秒）を挟む。「大津波」「深発地震」の
ような一般用語（辞書の `_terms`）は文中に溶け込む語なので対象外。

#### 繋ぎ目の音素長の引き直し

上記のとおり辞書該当語は前後と別々に VOICEVOX へ渡して繋ぎ合わせている（`buildAccentPhrases`）。
この渡し方だと前半が独立した 1 文として扱われ、その末尾（「震度5弱を」のように助詞で終わることが
多い）が文末と解釈されて母音がおよそ 2 倍に伸びる。地名の直前の助詞が間延びして聞こえる原因。

そのため繋ぎ合わせた後に `POST /mora_length` へ通し、全体の文脈で長さを引き直す
（`refineMoraLength`）。再計算されるのは長さだけなので、辞書で指定したアクセントは変わらない。

ただし `DICT_TRAILING_PAUSE` も引き直しの対象になり、読点相当の長さまで伸ばされてしまう。
そのため句と句の間（`pause_mora`）は、辞書由来・読点由来を問わずまとめて元の値へ戻している。
引き直しに失敗した場合は助詞が伸びたまま読み上げるだけで、読み上げ自体は止まらない
（失敗は `log.debug` に残る。ただし割り込みによる中断は記録しない）。実測した秒数は
`refineMoraLength` のコメントに残してある。
...
- 音声合成そのものの失敗パス（`synthesizeChunk` の `catch`）と `/accent_phrases` の取得失敗に
  ログがなく原因追跡ができない（辞書の取得失敗と `/mora_length` の引き直し失敗は `log.debug` に残る）
...
- 2026-08-19: 句区切り辞書に載った地名の直前の助詞が間延びする問題を解消。辞書該当語の前後を
  ...
  音素長を引き直すようにした（§3「繋ぎ目の音素長の引き直し」）。あわせて辞書該当語の直後に挟む
