...
| `SpecialInfoBanner`（南海トラフ臨時情報・後発地震・南海トラフ関連解説情報のバナー） | 下。**左右分割時のみ**（縦積み時は地図の下につまみ・パネル・ナビが続き、画面下端には届かない） |
...
`SpecialInfoBanner` は 3 枚（南海トラフ臨時情報・後発地震・南海トラフ関連解説情報）が同時に
出うるため、余白を持つのは**下端に接する最後の 1 枚だけ**（上に来る帯にも入れると帯と帯の間に
隙間ができる）。判定は `:last-child`（`side:last:` ユーティリティ）に任せており、枚数が
増えても JS 側で並び順を数える必要はない。

3 枚の役割と重さは次のとおり。上から重い順に並べる。
...
解説情報だけ閉じるボタンを持つのは、平常時にも定期的に届く電文だから（読み終えた帯を手で
片付けられるようにしている。発表頻度は [data-sources-spec.md](data-sources-spec.md) §2）。
閉じた電文の id は `localStorage` に残すのでリロードでも復活しない。新しい解説情報が届けば
id が変わるため、そちらは表示される。
...
│   │   ├── SpecialInfoBanner/ # 南海トラフ臨時情報・後発地震・南海トラフ解説情報のバナー
