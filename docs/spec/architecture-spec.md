...
`sideNarrow`（既定 320px）で顕在化するので、`EarthquakeTab/EarthquakeCard.tsx` の一覧カードでは
幅を押し広げないよう、長い震源地名を `flex-shrink-0` を使わず折り返している。
...
ルート要素（`App.tsx`）はビューポート全体を `fixed inset-0` で覆う。高さ単位（`100dvh` など）で
指定しないのは、iOS でホーム画面から起動した PWA だとセーフエリアを除いた高さが返ることがあり、
画面下端に届かず背景色の帯が残るため。**高さのユーティリティを併記しないこと** —
`height` が入ると CSS の規則で `bottom: 0` が無視され、この対策が効かなくなる。

画面端に置くものは、その端のセーフエリア（ノッチ・ホームインジケータ・ステータスバー）を自分で
避ける。地図は背景なので端まで広げてよいが、操作する要素と読ませる文字は重ねない:

| 要素 | 避ける端 |
|---|---|
| `IconNav` | 下・右。上は**左右分割時のみ**（縦積み時のナビは最下部にあり、上端を空ける理由がない） |
