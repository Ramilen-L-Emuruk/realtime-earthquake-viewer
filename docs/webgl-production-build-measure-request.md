...
- **`domWrites` が本番ビルドでも 1万回/秒台なら、「段階0 は移動中に効かない」が確定する。**
  ...
  **→【レビュー(546ee9f) で緩和】** 「変わるなら計測器を疑え」は強すぎた。開発機で本番ビルド静止の
  `domWrites` が dev の 1041→11.6 と桁で下がった観測が出た以上、**dev モードの React 再レンダリングが
  `MutationObserver` で拾われる DOM 書き込みを増やす経路は実在しうる**。実機で本番ビルド移動中に下がっても、
  計測器を疑う前にまずこの経路（dev モード由来）を検討する。ただし開発機の 1041 は元々実機 dev(31〜36) と
  30倍ずれた異常値で、実機の dev→prod がどう動くかは未検証。
