...
> - **【未対応】LOW 5**: 点が 1 つも無いフレームで FBO 束縛が漏れる（`3a42d36` の検証中に発見）。
>   PoC では顕在化しないが**本番移植で踏む**（下記「5.」）
...
5. **【LOW】5** FBO 束縛の漏れ（`render` 末尾の復元に `bindFramebuffer(mainFBO)` を 1 行追加）
   — PoC の結論と実機計測には影響しないため**本番移植の着手時までに**直せばよい
...
## 【LOW】5. 点が 1 つも無いフレームで FBO 束縛が漏れる（`3a42d36` 時点で**未対応**）
