...
> - **【対応済み】MEDIUM 6**（`c471303` 検証で追加）: 負荷軸③（17,250点）の複製は**可視の塗り面積を増やさない**
>   （②50,132px → ③50,534px・点数3.95倍に対し +0.8%）。`loadPositions` の「断片数など計測コストには
>   影響しない」は事実と逆だった（断片数は点数比例で増える。増えないのは可視面積）。コメントを修正し、
>   計画書 §6 の結論にも留保を反映した（下記「9-3」）。**実機スイートの結論に影響なし**
...
## 9. 対応の検証（`c471303`）— 負荷軸3段の実測と、複製の性質

`c471303` を開発機（Chrome / DPR 1 / **170Hz** / canvas 1904×929）で検証した。**HIGH 1・MEDIUM 3・情報5 は
解消を確認。HIGH 2 の負荷軸も指定どおり動く。** ただし負荷軸③の性質について新たに MEDIUM 1件（下記 9-3）。

### 9-1. HIGH 1 の回帰確認（内部経路でも一致）

`gl.drawElements` / `gl.drawArrays` をフックして POINTS 頂点数を積算した実測。**PoC 内部の
`genLevels` を通る経路**（手動トグル `custom-update 毎秒` を1周期踏ませる）でも確認した。

| 経路 | 描画点数 |
|---|---|
| `repaint-only`（`initLevels`） | **1,725** |
| 手動トグル経由（内部 `genLevels`） | **1,725** |
| 停止後（`initLevels` へ復帰） | **1,725** |

修正前の 1,465 から揃った。差分の土台は正しくなった。

### 9-2. 負荷軸3段の実測（点数・可視面積・等価性）

各段で「全点 index 0（カスタムレイヤーが何も描かない状態）」と「index 1〜6 ランダム」の
フルフレーム `readPixels` を取り、差のあるピクセル数＝**カスタムレイヤーが可視に塗った面積**を数えた。

| 段 | クエリ | 点数 | 描画点数 | **可視の塗りピクセル** | 点数倍率 | 塗り倍率 | 1点あたり | custom vs naive |
|---|---|---|---|---|---|---|---|---|
| ① | （既定） | 1,725 | 1,725 | 26,050 | 1.00 | 1.00 | 15.1 px | **差 0** |
| ② | `?points=4372` | 4,372 | 4,372 | 50,132 | 2.53 | 1.92 | 11.5 px | **差 0** |
| ③ | `?points=17250` | 17,250 | 17,250 | **50,534** | 10.00 | **1.94** | 2.9 px | **差 0** |
| ④ | `?points=99999` | **65,535**（クランプ） | 65,535 | — | — | — | — | — |

- **3段すべてで custom / naive の描画結果はピクセル完全一致**（フル画面比較・差 0）。等価性に回帰なし
- ④ クランプは正しく動作。最大 index 65,534 で Uint16 の範囲内、`glError` 0、`console.warn` 1件
- ①→② で既に塗り倍率が点数倍率を下回る（2.53倍 → 1.92倍）。これは点密度が上がれば必然（複製とは無関係）

### 9-3. MEDIUM 6. 負荷軸③の複製は可視面積を増やさない（コメントの一文が事実と逆）

**②50,132px → ③50,534px（+0.8%）。点数を3.95倍にしても可視の塗り面積は動かない。**
③の座標は②の 4,372 位置を巡回複製したものなので**ユニーク位置が 4,372 を超えられず**、
②の時点でほぼ塗り尽くされた同じ位置を重ね塗りしているだけだからである。

`loadPositions` のコメントにこうある。

> 位置の重複は許容する（…）drawcall・**断片数など計測コストには影響しない**

**断片数は影響を受ける。** 点数10倍なら断片シェーダの実行回数も約10倍になる（各点が `gl_PointSize²`
相当の断片を生成し、同一ピクセルへの重複書き込みも GPU の仕事として発生する）。
影響しないのは**可視の塗り面積**だけである。正しい整理は以下。

| 点数を10倍にすると | 挙動 |
|---|---|
| 頂点シェーダ実行・断片シェーダ実行・ブレンド演算・index 転送量・`apply` の CPU | **約10倍**（狙いどおり） |
| **可視の塗り面積** | **据え置き**（②で飽和済み） |

#### なぜこれが結果の解釈に効くか

- **③は方式間の相対比較には有効**。両方式に同じ条件がかかり、可視面積が固定される分、
  差が出た場合に「純粋に更新コスト（頂点処理・転送・CPU）の差」と帰属できる。交絡が少ない良い軸である
- **③は本番負荷の予測には使えない**。本番の観測点は重複しないため、同じ 17,250点でも可視面積が
  全く違う（本番相当は①②のみ）
- したがって **③で分離点が見つかっても「本番で観測点が増えたらこうなる」とは言えない**。
  逆に **③で分離しなくても「描画負荷10倍まで余力あり」とは書けない**（描画は 1.94倍のまま）。
  この場合の正しい主張は「**更新 CPU・頂点処理・転送を10倍にしても差が出ない**」である

#### 提案

コードの修正は不要（この性質は測定上むしろ望ましい）。**コメントの一文を直し、③の結果を
計画書へ書くときの留保として残す**だけでよい。

- 「断片数など計測コストには影響しない」→「**断片数は点数に比例して増える。増えないのは可視の塗り面積**」
- ③の結論は「描画負荷10倍」ではなく「**更新負荷10倍・可視の描画量は据え置き**」と書く

**【対応済み】** `loadPositions` のコメントを上記の整理に修正し、計画書 §6 の結論（「描画負荷10倍まで
余力あり」ではなく「更新CPU・頂点処理・断片処理を10倍にしても天井内」）にも留保を反映した。

### 9-4. 実機スイートへ進める状態か

**進めてよい。** HIGH 1 が解消して差分の土台が揃い、負荷軸も3段動く。等価性の回帰もない。
9-3 はコメントと結果表現の話でコードの正しさには影響しないため、実機計測をブロックしない。

実機での手順（項目6 に倣い**各水準3回**。ミリ秒スケールのばらつきが約2倍あるため単発では信頼区間が不明）:

```js
// ?points= を変えてページを開き直し、各水準で実行する
await window.__runSubRtSuite('surface-go2-subrt-p1725', 12000)
await window.__runSubRtSuite('surface-go2-subrt-p4372', 12000)
await window.__runSubRtSuite('surface-go2-subrt-p17250', 12000)
```

読み方の指針:
- **`repaint-only` との差**が更新の追加コスト（`updateFrame` で読む。`apply` は差分不要）
- **`custom-update` と `naive-rebuild` の差**が方式の優劣。差が分解能・ばらつきに埋もれたら
  「両方式とも軽い」が正しい結論で、「index 方式に優位が無い」ではない
- ①②が本番相当、③は方式比較専用（9-3 の留保つき）
