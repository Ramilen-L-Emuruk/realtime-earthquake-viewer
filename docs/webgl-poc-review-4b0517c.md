...
> - **【完了】改訂版スイートで実機計測を実施し、層 B の律速判定が決着**（19計測・証跡 `perf-2026-07-25T10-5*`）。
>   塗り面積 4.0 倍・線幅 10 倍まで上げても vsync 天井を破らず＝**律速に到達しない**。
>   `baseline-warm` 対照 16.9ms によりウォームアップ交絡も確定。結果は計画書 §6 結果記録（3回目）
...
2. ~~その後に実機で `__runLayerBSuite('surface-go2')` を実行~~ → **実施済み（証跡 `perf-2026-07-25T10-5*` 19件）**

**このレビュー綴りは完了。** 改訂版スイートは実機で 19計測を完走し、**層 B の律速判定は決着した**
（塗り面積 4.0 倍・線幅 10 倍まで上げても vsync 天井を破らず＝律速に到達しない）。
`baseline-warm` 対照が 16.9ms を返し、**1・2回目の zoom 33.3ms がウォームアップ交絡だったことも確定**。
結果と判断は計画書 §6「結果記録」の 3回目エントリ、観点は
[webgl-layerb-verification-points.md](webgl-layerb-verification-points.md) 付録「3回目」。

つまり本レビューで直した 3 件（z6 漏れ・`verts-thin` 非対称・開始 zoom）は、
**いずれも「直さなければ判定を誤っていた」箇所**だった。特に z6 漏れを残していれば
baseline zoom に再タイル化コストが乗り続け、交絡仮説の対照実験が成立しなかった。
