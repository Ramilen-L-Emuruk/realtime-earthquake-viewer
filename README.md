...
- **緊急地震速報（EEW）**: 発報時に予報円・震源・警報区域を表示、気象庁が発表した地域ごとの主要動到達予測をカウントダウン
...
- **凡例**: いま地図に描かれている色の意味を左下に常時表示（見出しを押すと畳めます。設定で非表示にもできます）
...
- **共有カード**: 表示中の地図を、見出しと出典を添えた 1 枚の画像にして共有・保存（表示中の内容の概要を本文として添えます）
...
- 地震波の走時: 気象庁「走時表（JMA2001）」（https://www.data.jma.go.jp/svd/eqev/data/bulletin/catalog/appendix/trtime/trt_j.html ）を加工して作成（公共データ利用規約 第1.0版）
