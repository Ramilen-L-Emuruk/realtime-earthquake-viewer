...
- **地震情報タブ**
  ...
  - 日本地図に **各観測点の震度を震度別の色付きドットで表示**（震源は×印）。観測点が多数になっても軽快なよう **Canvas で描画**（色・大きさで震度を表現）
  - **引き〜中間ズームでは一次細分区域ごとの最大震度に集約**（観測点を含む区域を判定し、区域の中心に最大震度を数字で表示・同じ震度色で塗りつぶし）。寄ると観測点ごとの色ドット表示に切り替わる
...
- **リアルタイムタブ**
  - 各観測点（約1725点）のリアルタイム震度を Leaflet 地図に毎秒更新で **Canvas の色付きドット**で表示（DOM 非使用で軽量。Yahoo リアルタイム震度の JSON データを使用・HTTPS）。**震度0未満（計測震度0.0未満）は非表示、震度0は小さい灰色の点、震度1以上は気象庁の震度配色＋震度で大きさを変える**（地震・EEW を含む全ての震度表示で統一）
  - EEW 非発報時にタブを開くと日本全体を表示
  - 緊急地震速報の発報時は **予報円（青=P波 / 赤=S波）と震源（震源地名ラベル付き）**を地図に重ねて表示し、右パネルに EEW 情報カードを表示
  - 強震モニタで近接する複数点が急上昇し震度2相当以上の揺れを検知すると、右パネルに **「揺れを検知中」カード**を表示（推定最大震度を震度スケール色付きで表示）
  - 右パネルに震度カラースケール凡例・注記

- **津波情報タブ**
  - 大津波警報 / 津波警報 / 津波注意報をリアルタイム表示
  - **警報・注意報の海域を地図の海岸線に等級色で描画**（大津波警報＝紫／警報＝赤／注意報＝橙）し、対象区域へ自動ズーム。**発報中は地震情報・リアルタイムタブでも海岸線を常時表示**
  - 到達予想時刻・予想高さを表示
...
- **緊急地震速報 (EEW)**
  ...
  - **対象地域（一次細分区域）を予想震度（最大）の色で塗りつぶし**（解除で消去）
