...
- **リアルタイムタブ**
  - 各観測点（約1725点）のリアルタイム震度を Leaflet 地図に毎秒更新で色分け表示（Yahoo リアルタイム震度の JSON データを使用・HTTPS）。**震度0未満（計測震度0.0未満）は非表示、震度0は灰色、震度1以上は気象庁の震度配色**（地震・EEW を含む全ての震度表示で統一）
  - **震度1以上の各点に震度階級の数字をズームに依らず常時表示**（震度0は灰色の点のみ）
