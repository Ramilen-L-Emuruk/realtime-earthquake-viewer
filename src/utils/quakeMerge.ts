...
//
// **どちらの判定より先に、「完全版を受けた後の速報段階か」を見る**（`isSupersededByExistingCard`）。
// そちらが真なら、この表も発表時刻も見ずに据え置く。
//
...
/**
 * 既存カードが完全版で、`incoming` が速報段階か。
 *
 * **見出しの呑み込み（`normalizeReports`）と同じ定数で判定する。** 「このカードは何の電文か」を
 * 見出しと中身で別々に決めると、見出しは完全版なのにカードの色と中身だけ速報段階へ倒れる
 * （→ `mergeQuakeInto` の据え置き判定）。
 *
 * **訂正報（`issue.correct`）もここでは区別しない。** 一度は「訂正は内容を採る」側へ倒したが、
 * 取り下げた —— 速報段階の訂正報を採ると、**この関数が塞いだはずの不整合をそのまま再現する**。
 * 震度速報の粒度（区域のみ）で完全版を上書きするので観測点・市町村の段が消え、`issue.type` が
 * 速報段階へ倒れてカードの色が変わる一方、見出し（`normalizeReports`）は完全版のまま残る。
 * しかも震度を持たない訂正報（震源情報型）は下の優先度判定で結局据え置かれるので、
 * 「訂正は採る」が**半分しか成り立たない**状態になっていた。
 *
 * 実配信でこの組み合わせ（完全版の後に届く、速報段階の訂正報）は観測していない —— 控えの
 * アーカイブ 5 日分で完全版の後に届いた速報段階 8 件はすべて `InfoType` が「発表」で、
 * `InfoType` が「訂正」の地震情報自体アーカイブ全期間で 0 通（→ §6.2）。**実在しない形のために
 * 実在する不整合を作らない。** 代わりに据え置いた事実を `log.warn` で残し、観測されたら
 * 気づける形にしてある（→ `mergeQuakeInto` の据え置き判定）。
 */
function isSupersededByExistingCard(existing: JMAQuake, incoming: JMAQuake): boolean {
  return FULL_REPORT_TYPES.includes(existing.issue.type)
 ...
 * §6.3「据え置いた電文は、音・読み上げ・タイトル・タブ移動・選択・分布モードのクローズも
 * 起こさない」）。
 *
 ...
 *
 * 代償として、**完全版を受け取った後に震度速報が届いても見出しには出ない**（記録そのものは
 * 残る）。**この順序は気象庁の発表で実際に起きる** —— 近接した時刻に 2 つの地震があると、
 * あとから入電した区域の震度が先に検知していた地震へ紐づくため。そのときカードの中身は
 * `mergeQuakeInto` の `isSupersededByExistingCard` が据え置くので、**見出しにだけ出ない**。
 */
...
export function mergeQuakeInto(
  ...
  //
  // **ただし、既存が完全版なら速報段階の電文は時刻を問わず据え置く**（`isSupersededByExistingCard`）。
  // 下の「実震度を持つ電文どうしは時刻で判定する」より先に効く。理由は 2 つ。
  //
  // 1. **粒度が違う。** 震度速報は区域の最大震度しか持たない（電文の `CodeDefine` が `Pref/Code` と
  //    `Pref/Area/Code` の 2 つだけで、`City` も `IntensityStation` も無い）。採ると市町村・観測点の
  //    段が丸ごと消え、震度一覧が 4 段から 1 段へ落ちる。
  // 2. **実データで、完全版の後に来た速報は正しい情報を持っていなかった。** DMDATA のアーカイブ
  //    5 日分（電文 386 通・地震 274 件）を走査すると、完全版の後に速報段階が届いたのは 8 件で、
  //    うち 7 件が完全版と食い違う区域・震度を主張していた。**その 7 件はいずれも、後続の完全版で
  //    裏付けられていない**（気象庁はその地震について続報を出していない）。実例として
  //    2024-11-26 の EventID=20241126224512 は、大阪府北部 M2.4 深さ12km の地震に対して
  //    福井県嶺南・滋賀県北部の震度3 を伝えてきたが、そこは震源から 80〜95km 離れていて
  //    この規模では届かない。同じ 2 区域・同じ震度は 1 分後の石川県西方沖 M6.6
  //    （EventID=20241126224709）の電文にも載っており、近接した時刻の別の地震の揺れが
  //    紛れ込んだもの。気象庁は訂正報も出していない。
  //
  // **完全版より前の震度速報は従来どおり受け入れる。** 下の能登 2024/1/1 の事例（新潟県佐渡が
  // 増えた続報）は完全版が届く前の話なので、この据え置きには掛からない。
  //
  // **時間の窓は置かない。** 根拠にした 8 件の時間差は完全版の 1〜81 秒後（全件 2 分以内）で、
  // 抑制の範囲（無期限）のほうが広い。それでも窓を置かないのは、**窓を置けば「窓の外の速報は
  // 採る」と決めることになるのに、窓の外で正当だった速報を 1 件も観測していない**ため——値を
  // 決める根拠が無い。据え置いて失うのは「完全版より後に観測された区域の震度」で、それが本物なら
  // 気象庁は完全版の続報で確定させる（上記 7 件はいずれも続報が出ていない）。
  // **窓の外の速報が正当だった例が見つかったら、この判断を見直すこと。**
  //
...
// 線形探索で足りる。
//
// **既知の限界: 同じ分（time の精度限界）に同じ段階の電文が複数ある場合、newQuakes の
...
// だけで判定するため、同じ分に複数の震度速報（再送等）が並べば、後に置いた方が勝つ。
//
// **完全版（震源・震度情報 / 各地の震度情報）と速報段階（震度速報 / 震源情報）のあいだでは
// 逆転しない。** 完全版を受けた後の速報段階は据え置くため（`isSupersededByExistingCard`）、
// 入力配列で詳しい方が先・粗い方が後でも粗い方には後退しない。
//
// 呼び出し側（`services/dmdataReplay.ts` の `orderedForMerge`）は「発表時刻の昇順 → 同値なら
// 種別優先度の昇順」で並べ直す。**段階をまたぐ逆転が据え置きで消えた後もこれは残す** ——
// 同じ段階どうしの並びは上のとおり依然として入力順に依存する。呼び出し側で結合順序を変える
// 場合はこの前提を崩さないこと。
