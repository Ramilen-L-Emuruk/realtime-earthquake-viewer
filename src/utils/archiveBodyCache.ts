 ...
 *
 * **後者を見落とすと足りない。** `fetchDmdataQuakeHistory` は窓の全日を
 * `Promise.all(targets.map(...))` で**先に一括して落とす**（件数による早期打ち切りは、そのあとの
 * パースの段にしかない）。しかも「もっと見る」は押すたびに `maxDays` を 7 日伸ばして窓全体を
 * 組み直すので、**5 回押すだけで 32 本を越える**。
 *
