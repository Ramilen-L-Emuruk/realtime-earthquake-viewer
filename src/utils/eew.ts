 ...
 *
 * 区域別予想を先に見るのは、地域ごとの内訳のほうが細かいため。区域の列挙が始まるのは電文全体の
 * 予想が震度 4 以上になってからで、震度 3 以下の予想は区域が空のまま電文全体の値だけが載る。
 * **区域そのものに下限は無い**（震度 2 の区域も並ぶ）。この順序で値を取りこぼさないことの裏付けは
 * docs/spec/eew-spec.md §4「区域に載る予測震度に下限は無い」。
 *
 ...
 * 「値が変化したかどうか」の判定に使う。
 *
 * 数値差ではなく段階数で測る理由: 6弱(55)→7(70) は数値差15、6強(60)→7(70) は数値差10で、
 * 同じ「1段階の変化」でも数値差が不均一（5弱/5強・6弱/6強だけ5刻み）。閾値判定を数値差で
 * 行うと、この2つを同列に扱えない。
 */
...
/**
 * 震度の跳躍幅がこの段階数以上なら「変化した」とみなし、長く安定を待つ（判定は `eewScaleJumpSteps`）。
 *
 * 値は 1（＝1段階以上の変化なら常に長く待つ）。かつては「2段階以上の急な跳躍だけ長く待ち、
 * 1段階の変化は短い猶予で確定してよい」という設計だったが、2024/01/01 能登本震の第13報
 * （6強→7、1段階の変化）が実データで反証した：直前の確定値からの跳躍幅が1段階だったために
 * 短い猶予（300ms）しか与えられず、608ms後に届いた6強への訂正報を待てずに、0.6秒しか
 * 存在しなかった震度7がそのまま確定・読み上げられてしまった。実データ8件での比較検証では、
 * 閾値を1に下げても最終的な読み上げ内容が変わったケースはなく（確定タイミングが数秒〜
 * 十数秒遅くなるのみ、上限は `EEW_PHASE2_STABILITY_MAX_WAIT_MS` でキャップされる）、
 * むしろ日向灘・岩手県沖の類似の瞬間的な値の誤読も併せて防げることを確認している。
 */
export const EEW_PHASE2_SCALE_JUMP_STEP_THRESHOLD = 1
/**
 * 変化なし（跳躍幅0段階）での安定待ち時間。閾値が1のため、これに該当するのは
 * 初出値（サイクル開始時点の baseScale=currentScale）か、進行中の安定待ちサイクル中に
 * 値が直前の確定値へ厳密に一致して戻った場合のみ——「緩やかな変化」全般ではなく、
 * 実質的には「変化していない」ケースに限られる。この復帰検出が、能登本震13報の
 * 回帰テストで震度7を退ける核心（608ms後の6強への復帰が SMALL=300ms で即確定し、
 * まだ発火していない震度7の LARGE タイマーを追い越す）。
 */
export const EEW_PHASE2_STABILITY_SMALL_MS = 300
/** 変化あり（跳躍幅1段階以上）での安定待ち時間。 */
...
const EPICENTER_RANK_LABEL: Readonly<Record<number, string>> = {
  ...
  6: '防災科研システム（5点以上）',
  // **「海域」「内陸」だけにしないこと。** 解説資料は「EPOS（海域〔観測網外〕）」
  // 「EPOS（内陸〔観測網内〕）」と書いており、括弧書きが**観測網の外か内か**を言っている。
  // 落とすと、その区別が画面から消える（「海域」だけでは何と対比しているのか分からない）。
  7: 'EPOS（海域〔観測網外〕）',
  8: 'EPOS（内陸〔観測網内〕）',
}
 ...
 * 片方だけ電文順のままだと、聞いた順と画面の並びが食い違う（津波の区域の並びと同じ規律。
 * → `docs/spec/audio-tts-spec.md` §4「津波の区域の並び順」）。
 *
 * 表に無い名前（気象庁が地方予報区を増やした場合）は**末尾へ回して元の順序を保つ** ——
 * 捨てると警報の対象が画面からも声からも消える。
 */
export function sortEewWarningRegions(regions: readonly string[]): string[] {
  const rank = (name: string) => {
    const i = EEW_WARNING_REGION_ORDER.indexOf(name)
    return i < 0 ? EEW_WARNING_REGION_ORDER.length : i
  }
