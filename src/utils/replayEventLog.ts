...
export type ReplayTelegramSkip =
   ...
   * 決めている**。
   *
   * **「もっと見る」経路では、電文の中身を問わず経路単位で使う。** 地震カードの一覧を
   * 遡って厚みを増やす「もっと見る」は、南海トラフ臨時情報・後発地震注意情報等の帯を
   * 画面へ一切反映しない（古い日の電文で現在表示中の最新の帯を上書きしてしまう副作用を
   * 避けるため）。この場合の除外理由は電文の内容ではなく「この取得経路自体が反映しないと
   * 決めている」ことだが、他の値（`notApplied` は受信側の事情による棄却、`silentReplayInit`
   * は画面には反映される場合の値）のどちらにも当てはまらないため、こちらへ含める
   * （→ `useEarthquakes.ts` の「もっと見る」履歴バッチ処理）。
   */
  | 'notDispatched'
  /**
