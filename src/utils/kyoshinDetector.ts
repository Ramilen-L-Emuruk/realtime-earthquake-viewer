...
export const PARAMS = {
   ...
   * ためで、2 に留めるのは「震度3の単点は信じない」（§18）を維持するため。§26。
   */
 ...
 */
/**
 * 連結成分をイベント候補として認める最小点数。成分内の最大震度が高いほど緩める（§26）。
 *
...
export function step(
  ...
  // 成分をイベント候補として認める点数は、成分内の最大震度で緩める（§26）。
...
function updateEventMetrics(
  ...
  // 高震度 fast path（§20・§26）: HIGH_CONFIRM_INTENSITY(震度3) 以上に達したメンバーが
  ...
  //    分布を、震度3のバーで再び通さないため。ただし成分点数の緩和（§26）で認められた小さな成分
  ...
  // 成立する。§26）。単点・2点の成分は毎フレーム requiredClusterSize を満たし直す必要があるため、
