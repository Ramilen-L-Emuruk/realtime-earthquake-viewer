 ...
 *
 * 緊急地震速報の予想値の鍵（`4程度以上`・`5弱程度以上`）も数字で始まるので同じ扱いにする。
 * 読み上げ文では直前が「震度」「階級」なので数字が隣接する形は作れないが、**それは震度ラベルが
 * 1 桁であることに寄りかかった前提**で、気象庁が書いた文も読み上げに乗る。数字が並ぶ本文
 * （「12程度以上」）が来たときに値そのものを割らせない。ここも後ろは鍵が閉じているので前だけ見る。
 */
const NUMERIC_KEY = /^(?:[0-9]{1,2}(?:日|時|分|分頃|分ころ)|[0-9](?:弱|強)?程度以上)$/
