...
export function earthquakeToSegments(
  ...
  if (type === '震度速報') {
    ...
    // 最大震度だけでも伝え、そうでなければ黙る。
    if (saidSomething) return []
    return [plain(`${prefix}最大震度${intensityText(maxScale)}を観測しました。`)]
  }
  ...
  if (!isEpicenterOnly) {
    segments.push(...buildRegionSegments(event.points, maxScale, opts, hypocenter, spoken))
  }
  return segments
}
