 ...
 *
 * `announceWarning` が真のときだけ「警報。」を前置きする。**その EEW で警報だと初めて伝える
 * ときだけ真にすること**（判定は呼び出し側）。続報のたびに付けると、値を読み直すだけの報でも
 * 毎回「警報」が挟まって耳に障る。一方で完全に省くと、予想震度が据え置きのまま severity だけ
 * 確定する続報で区分の変化が無言になる。初回だけ言えば両方を満たせる。
 *
 * 「〜に切り替わりました」のような差分の言い方は使わない。前の区分を一度も声に出していない
 * 場面（初報から警報だった場合・予想値を読む前に格上げが届いた場合）で、何から変わったのかが
 * 伝わらないため。
 *
 * **「特別警報」は読み上げない。** 特別警報の条件を満たしていても「警報。」と読む。気象庁が
 * 発表時にこの名称を用いないため（根拠と条件は docs/spec/eew-spec.md §4）。画面表示・ブラウザ
 * 通知・通知音は内部の重大度区分として 2 段階を保つ。
 *
 * 引き上げ専用の短句（「震度6弱に引き上げ。」）は持たない。同じ形で言い直せば足りるうえ、
 * 差分の言い方は「基準にした値を実際に発話したか」に依存し、割り込みで消えた発話を基準に
 * すると一度も声に出していない値からの引き上げを語ることになるため。
 */
export function eewIntensityToText(event: EEWAlert, announceWarning = false): string {
  let text = announceWarning ? '警報。' : ''
