...
describe('earthquakeToSegments: 続報は差分だけ読む', () => {
  ...
  it('未入電の区域は「〜では震度5弱以上と推定されます」で読む', () => {
    const state = createQuakeSpokenState()
    ...
    expect(text).toBe('震度速報。宮城県北部では震度5弱以上と推定されます。')
  })
  ...
  it('観測値と未入電が混ざれば文を分けて両方読む', () => {
    ...
    expect(text).toContain('宮城県北部では震度5弱以上と推定されます。')
  ...
  it('同じ未入電の続報では読み直さない', () => {
    ...
    expect(joinSegments(second)).not.toContain('以上と推定されます')
  })
  ...
  it('未入電の区域が上限を超えたら「ほかN地域」で省略を伝える', () => {
    ...
    const text = joinSegments(earthquakeToSegments(quakeOf(UNRECEIVED_3, 45), opts, true, state))
    expect(text).toContain('ほか2地域では震度5弱以上と推定されます。')
  })
