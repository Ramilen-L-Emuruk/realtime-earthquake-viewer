 ...
 */
type BeepLevel = 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7
/** 段ごとに 1 つずつ値を持つ表。長さが `BeepLevel` の値域と一致することを型で保証する */
type ByBeepLevel<T> = readonly [T, T, T, T, T, T, T, T]

// 強震モニタ index → 段のマッピング（インデックスは 0〜20、計測震度 = index * 0.5 - 3.0）。
//
// **気象庁の階級 10 段のうち、まとめてよいのは下の 3 つだけ。** 震度0〜2 は「揺れを感じ始めた」
// 以上の意味を持たないため 1 段に畳んでいる。**上は 1 対 1 で対応させること**——この関数は
// 音を選ぶだけでなく、`useKyoshinAlerts` が「段が上がったか」で検知音・タブ移動・揺れフォーカスの
// 3 つを起動する判定にも使う。畳んだ範囲では悪化しても何も起きない。
export function kyoshinLevel(index: number): BeepLevel {
  if (index >= 19) return 7  // 震度7   (計測震度 6.5+)
  if (index >= 18) return 6  // 震度6強 (計測震度 6.0〜6.5)
  if (index >= 17) return 5  // 震度6弱 (計測震度 5.5〜6.0)
...
/**
 * 震度更新音の段階ごとの深刻度。**8 段階を 3 dB 刻みでは表せない**ので、この表が段を刻む。
 *
 * 震度6弱までは 1 dB ずつ緩やかに上げ、**震度6強と震度7 では 2 dB ずつ開く**。音量だけで
 * 8 段を刻もうとすると、下の段が聞こえないほど小さくなるか、上の段が警報を覆うかのどちらかに
 * なる。**勾配を作っているのは音量だけではない**——音数と `deep`（震度5弱以上でサブ
 * オクターブが厚くなる）も効いている。**段ごとの内訳は下の `BEEP_PATTERNS` が持つ**ので、
 * 音数をここに書き写さないこと（片方だけ更新されて食い違う）。
 */
const BEEP_SEVERITY: ByBeepLevel<number> = [
  SEVERITY.update, 0.281, 0.316, 0.355, 0.399, 0.449, 0.565, SEVERITY.warning,
]

// 震度が上がるほど音数が増え（2 → 8 音）、間隔が詰まる。**この勾配が段階の重さを伝える**ので、
// 音色を変えるときも数値の並びは崩さないこと。音量は BEEP_SEVERITY が別に持つ。
// 実測の勾配は端から端まで（震度2以下 → 震度7）で合計 10.5 dB。
//
// 音程は A4・C5・D5・F5・G5・A5・C6（440〜1047Hz）。**最上段だけ 880Hz の外へ出る。**
// 下位 6 段は音数（2 → 6 音）で分かれるが、上位は 6〜8 音まで来て 1 音の差が聞き取れない。
// 音程で分けようにも、440〜880Hz に収まる 6 音は震度4 の時点で出そろっており、震度6弱
// （G5-D5）と 6強（A5-D5）はそれを反復しているだけ。震度7 へ割り当てる音が残らないため、
// **最上段だけ C6-C5 へ上げて下の段と音を共有しないようにしている。**
//
// 1047Hz は揺れ検知のマリンバ（880・1108Hz）と近いが、音色が正反対なので混ざらない
// （矩形波の連打とフィルタを掛けた木琴の単発）。**1047Hz より上へは上げないこと**——この
// 一族の音域は 659〜1318Hz から 2 度下げてきた経緯がある（audio-tts-spec.md §2）。
const BEEP_PATTERNS: ByBeepLevel<BeepPattern> = [
  { freqs: [440, 587],                                   interval: 0.180, duration: 0.280, deep: false }, // 震度2以下
  { freqs: [587, 699, 784],                              interval: 0.150, duration: 0.220, deep: false }, // 震度3
  { freqs: [587, 699, 784, 880],                         interval: 0.130, duration: 0.200, deep: false }, // 震度4
  { freqs: [587, 699, 784, 880],                         interval: 0.120, duration: 0.180, deep: true  }, // 震度5弱
  { freqs: [699, 523, 699, 523, 699],                    interval: 0.110, duration: 0.170, deep: true  }, // 震度5強
  { freqs: [784, 587, 784, 587, 784, 587],               interval: 0.100, duration: 0.160, deep: true  }, // 震度6弱
  { freqs: [880, 587, 880, 587, 880, 587, 880],          interval: 0.090, duration: 0.150, deep: true  }, // 震度6強
  { freqs: [1047, 523, 1047, 523, 1047, 523, 1047, 523], interval: 0.085, duration: 0.140, deep: true  }, // 震度7
]
