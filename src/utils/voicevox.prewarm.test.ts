...
//
// 通知音と声が重ならないよう電文ごとに 0.5〜2.8 秒の間を置いているため、その間に最初の
...
describe('先行合成', () => {
  ...
  it('先行合成した音声を使うので、再生時に合成し直さない', async () => {
    ...
    await speakWithVoicevox('http://x', text, 1, 1, undefined, pre)
  ...
  it('テキストが違う先行合成は使わない', async () => {
    ...
    await speakWithVoicevox('http://x', '地震情報。', 1, 1, undefined, pre)
  ...
  it('ほぼ同時に 2 件届いても、先に始まった側の先行合成を後から始まった側が打ち切らない', async () => {
    ...
    const quakePlay = speakWithVoicevox('http://x', '地震情報。', 1, 1, undefined, quake)
    // 直後に長周期の再生が始まっても、地震情報がこれから使うものは残す
    const lpgmPlay = speakWithVoicevox('http://x', '長周期地震動情報。', 1, 1, undefined, lpgm)
