...
export const MAP_LAYER_ORDER = [
  ...
  'subregion-hit',
  // 夜の側の重ね塗り（DayNightGL）。**地形だけに掛ける。** 境界線・震度の面より背面に置くのは、
  // 夜の面が情報を沈めないため（震度の面は地形の上・境界線の下にあるので、ここより前面に置くと
  // 震度の色が夜側で暗くなる）。夜側でも県境と震度は読めたまま、陸地と海底地形だけが暗くなる。
  // 薄明の段は 1 枚のレイヤーの中で重ねる（重なりは同一レイヤー内でも累積するため、段数を
  // 変えてもここは動かない）。
  'day-night',
  // 観測点の震度を補間した面（地震モードで観測点表示のとき）。境界線より背面に置き、
  // 県境・区域境が面の上に見えるようにする（面は半透明だが、線は透かさず出したい）。
  'quake-intensity-surface',
  // 気象庁の推計震度分布図（IXAC41）。自前の面と**排他**なので隣に置く（同時には出ない）。
  // 境界線より背面という理由も同じ。
  'quake-estimated-intensity',
  'sub-borders',
  'pref-borders',
