...
describe('rasterizeEstimatedIntensity', () => {
  ...
  it('値の無い画素へ、隣接する画素の最も大きい値を広げる（縁で階級が落ちないように）', () => {
    const r = rasterizeEstimatedIntensity(makeData([cellAt(0, 0, 47)]), BIG)!
    // 周囲 8 画素は「無効」のまま値だけを持つ。線形補間で 0 が混ざるのを防ぐため。
    for (const [x, y] of [[0, 0], [1, 0], [2, 0], [0, 1], [2, 1], [0, 2], [1, 2], [2, 2]]) {
      expect(px(r, x, y)).toEqual([47, 0])
    }
  })

  it('広げるのは 1 画素だけ（連鎖させない）', () => {
    // 5x5 の中央にセル 1 つ。余白を足して 3x3 になるので、そもそも 2 画素離れた画素は無い。
    // ここでは「広げた値が次の判定に使われない」ことを、離れた 2 セルで確かめる。
    const cells = [cellAt(0, 0, 40), cellAt(4, 0, 70)]
    const r = rasterizeEstimatedIntensity(makeData(cells), BIG)!
    expect(r.width).toBe(7)
    // セルは x=1 と x=5。広がるのは隣接だけなので x=3 は空のまま。
    expect(px(r, 3, 1)).toEqual([0, 0])
    expect(px(r, 2, 1)[0]).toBe(40)
    expect(px(r, 4, 1)[0]).toBe(70)
  })
