 ...
 * 「下限〜`REGION_MAX_ZOOM`」）では、ペインが十分大きいと**下限が上限を追い越す**——ラベル下限は
 * 短辺 3012px で 6.4 に達する（4K を 50% 表示にした程度で届く）。この短辺は `LABEL_MAX_SPAN_KM` と
 * `REGION_MAX_ZOOM` の両方で動くので、**どちらかを変えたら測り直すこと**。
 *
 * MapLibre は `minzoom > maxzoom` を検証せずそのまま代入し、可視判定は下限と上限を独立に見るため、
 * **どのズームでも描かれないレイヤーが黙って出来上がる**（例外もログも出ない）。帯が潰れる前に
 * ここで止める。
 *
