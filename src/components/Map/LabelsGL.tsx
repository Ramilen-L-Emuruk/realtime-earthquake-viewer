...
// ため、**開始と目標が同じズームでも移動中は必ず下回る**。幅は移動距離に比例し、実測では 6km で
// 0.0004・34km で 0.015・92km で 0.10・230km で 0.48（2026-08-30）。
//
// 境界を 8 に置くとこの落ち込みが境界を割り、区域名ラベルがレイヤーごと消えていた——地震カードを
// 選ぶたびに約 1 秒まるごと消え、着地した瞬間に戻る。ズーム値は小数第 2 位まで見ても 8 のままなので、
// 帯をまたいだようには見えない。
//
// 0.1 の余裕で守れるのは**約 90km までの移動**。同じ地震の続報や近くの余震はこの範囲に収まる。
