...
//   下限〜6.4  : 地方ラベル（引きの画。日本全体フィットはこの帯に入る）
//   6.4 〜 7.9 : 県名ラベル
//   7.9 以上   : 一次細分区域名ラベル（寄りの画。自動フィットの寄り上限が 8 に達するペインでは
//                着地後もこの帯に入る）
//
...
//
// 地方名と県名の境目。**カメラの着地段（0.5 刻み）に乗せてはならない**——理由は下の
// CITY_LABEL_MIN_ZOOM と同じで、着地が 6.5 になるペイン（短辺 494〜698px）では flyTo の
// アンダーシュートで境界を割り、移動中だけ県名が地方名へ落ちる。6.6 ではなく 6.4 なのは、
// 着地 6.5 の画でいま出ているのが県名だから（上へずらすと地方名に変わってしまう）。
//
// **余裕は同じ 0.1 でも、こちらの帯が守れる距離は長い。** 落ち込みは視野に対する移動量で決まるため、
// 引いた画ほど小さくなる（zoom 6.5 での実測は 34km で 0.0014・89km で 0.0099・224km で 0.060。
// 同じ距離で zoom 8 の 1/6〜1/8）。
//
// export しているのは 2 つの関係を回帰テストで固定するため（gl/zoomConstants.test.ts）:
// 地方名の帯（可変の下限 〜 この固定の上限）が潰れないことと、この値が着地段から離れていること。
// 前者は可変の下限と固定の上限が同じ帯に同居しているのがここだけで、崩れ方が無症状（どのズームでも
// ラベルが出ない）なので外から見張る。
export const REGION_MAX_ZOOM = 6.4

// 一次細分区域名を出し始めるズーム。**カメラが着地しうる値（0.5 刻み）に乗せてはならない。**
//
// 自動フィットの着地ズームは 0.5 刻みに丸められ（観測点が 1 つの地震は camera.ts の snapZoomNearest が
// 丸めた寄り上限へ直行し、複数点のフィットは snapZoomDown が切り下げる）、大きなペインでは
// 寄り上限が ABSOLUTE_MAX_ZOOM = 8 にクランプされて 8.0 に張り付く。一方 flyTo は放物線を描いて飛ぶ
// ため、**開始と目標が同じズームでも移動中は必ず下回る**。
//
// 落ち込み幅を決めるのは**視野に対する移動量**で、距離そのものではない（同じ距離でも引いた画ほど
// 小さい）。移動量に対しては 2 乗に近く増える。この境界の近く（zoom 8）での実測は 34km で 0.011・
// 89km で 0.075・224km で 0.38（2026-08-30）。
//
// 境界を 8 に置くとこの落ち込みが境界を割り、区域名ラベルがレイヤーごと消えていた——地震カードを
// 選ぶたびに約 1 秒まるごと消え、着地した瞬間に戻る。ズーム値は小数第 2 位まで見ても 8 のままなので、
// 帯をまたいだようには見えない。
//
// 0.1 の余裕でこの帯が守れるのは**約 100km までの移動**。同じ地震の続報や近くの余震は収まる。
// それより遠くへ飛ぶときは視野そのものが大きく変わるので、途中で粒度が落ちても不自然ではない
// （flyTo が遠距離で大きく引くのは演出の一部で、消し切ることはできない）。
//
// export しているのは、この「境界と着地点をずらす」関係を回帰テストで固定するため
// （gl/zoomConstants.test.ts）。8 へ戻しても型チェックは通り、症状は移動中にしか出ない。
export const CITY_LABEL_MIN_ZOOM = 7.9
