...
export function ShareCardButton({ state, onClick }: ShareCardButtonProps) {
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={`flex h-9 w-9 items-center justify-center rounded-md border bg-slate-900/80 backdrop-blur-sm transition-colors ${
        state === 'error'
          ? 'border-red-500/70 text-red-300'
          : 'border-slate-600/80 text-slate-200 hover:bg-slate-800/90 disabled:opacity-60'
      }`}
    >
...
function StateIcon({ state, className }: { state: ShareCardState; className: string }) {
  ...
  if (state === 'error') return <AlertIcon className={className} />
