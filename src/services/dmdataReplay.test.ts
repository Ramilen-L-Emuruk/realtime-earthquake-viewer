...
import {
  fetchDmdataReplayEvents, fetchDmdataQuakeHistory, clearReplayCache, clearParseCachesForTest,
  filterPreWindowEvents, MAX_HISTORY_DAYS,
} from './dmdataReplay'

/**
 * 控えを全部空にする。
 *
 * **パース結果の控えは `clearReplayCache()` では捨てない**（内容に対して不変な鍵で引くため）。
