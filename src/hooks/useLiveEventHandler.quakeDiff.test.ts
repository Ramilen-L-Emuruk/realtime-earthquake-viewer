...
describe('地震情報の続報: 既読は声になった分だけ進む', () => {
  ...
  // 2026-09-20 に反転: 3 区域とも震度4＝この電文の最大震度なので、続報で初めて挙げる区域にも
  // 「最大」が冠される（初出の群で冠さなかった頃は「新たに震度4を」だった）。
  // **このブロックの主旨は「どこまで鳴ったかで既読が決まる」ことで、そこは変わらない。**
  // 規則そのものは `utils/ttsText.test.ts` の「最大」は群ではなく事実で決める 3 件が固定する。
  ...
  it('正: 途中で切られたら、鳴った区域だけが既読になる', async () => {
    ...
    handle(makeQuake(threeAreas))
    await settle()
    // 石川県加賀・富山県東部はまだ一度も声にしていない＝初出の群（「新たに」が付く）。
    // 震度4 はこの電文の最大震度に一致するので「最大」も冠す（群では決めないため）
    expect(spokenTexts()[1]).toBe('震度速報が更新されました。新たに最大震度4を石川県加賀、富山県東部で観測しました。')
  })
  ...
  it('安全弁: 読み上げ中の見直しでは、最終チャンクが鳴り始めていても完走とみなさない', async () => {
    ...
    handle(makeQuake(threeAreas))
    await settle()
    expect(spokenTexts()[1]).toBe('震度速報が更新されました。新たに最大震度4を富山県東部で観測しました。')
  })
