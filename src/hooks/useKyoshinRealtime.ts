 ...
 *
 * **リプレイとローカル履歴アーカイブでは、これが表示の更新間隔の粒度になる。** どちらの供給元も
 * 再生時刻より先のフレームを先に積むので（`createYahooPipelineSource` ／
 * `createLocalKyoshinArchiveSource`）、1 秒ごとのデータが「ここで 1 件ずつ放出される」形になり、
 * 間隔のばらつきはこの巡回の粒度に収まる。**ライブだけは常に「すでに過ぎた時刻」のフレームが
 * 届くので、投入時の同期ドレインで放出され、この巡回を待たない。**
 *
 * 取り出せるものが無ければ即座に戻るだけなので、細かくしても費用はほぼ無い。
 */
...
export function useKyoshinRealtime(
  ...
  useEffect(() => {
    ...
    source.start({
      enqueue: (frame) => {
        queue.enqueue(frame)
        // ライブのフレームは常にデータ時刻が過去なので、ここで即座に放出される。
        // 巡回（DRAIN_INTERVAL_MS）を待たせないことで、従来と同じ即時性を保つ。
        // 先に積む供給元（リプレイ・ローカル履歴アーカイブ）では時刻が来ていないので、
        // ここでは何も出ず、巡回が時刻の到来で 1 件ずつ流す。
        drain()
      },
      setStalled: (stalled) => {
