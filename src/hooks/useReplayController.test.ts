 ...
 * 取得結果の形をひとつ作る。
 *
 * `addLoss` は**結果オブジェクトを丸ごと受け取る**（項目を個別に渡す形だと、後から足した枠を
 * 渡し忘れても型検査が通るため）。テストから毎回 4 項目を書くと読みにくいので、
 * 既定を埋めるヘルパーを置く。**429 の見送りを見るテストは明示的に渡すこと。**
 */
function lossOf(
  skipped: number,
...
describe('addLoss', () => {
  ...
  it('同じアーカイブを二重に数えない', () => {
    let loss = createEmptyLoss()
    loss = addLoss(loss, lossOf(0, ['https://x/a', 'https://x/b']))
    loss = addLoss(loss, lossOf(0, ['https://x/a']))
...
describe('addFailedPrefetch', () => {
  it('区間ごとに積み上げ、他の損失には触れない', () => {
    let loss = addLoss(createEmptyLoss(), lossOf(2, ['https://x/a']))
