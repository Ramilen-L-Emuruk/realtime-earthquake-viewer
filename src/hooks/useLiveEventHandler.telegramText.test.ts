...
/**
 * 鳴っている発話を完了させ、待ち行列を進める。
 *
 * **本文は電文本体より下位の層**で、本体が鳴り終わるまで待つ（`speakNonEEW` の待ち合わせ）。
 * 本体を完了させないと本文の番が来ないので、1 通ごとにこれを挟む。
 */
...
const COMMENT = '＊印は気象庁以外の震度観測点についての情報です。'
...
describe('気象庁が書いた文の読み上げ（配線）', () => {
  ...
  it('本文が変わったら読み直す', async () => {
    ...
    handleLiveEvent(makeQuake({ id: 'quake-1' }))
    await drain()
    handleLiveEvent(makeQuake({ id: 'quake-2', varCommentText: '震源要素を訂正します。' }))
    await drain()
