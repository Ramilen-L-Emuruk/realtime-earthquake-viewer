...
export interface LiveEventMeta {
   ...
   *
   * 真なら通知音・読み上げ・ウィンドウタイトル・自動タブ切替を起こさない。**それ以外は
   * 従来どおり行う** —— カードの選択・震度分布モードを閉じること・「この報は見た」の記録は、
   * カードが内容を採ったかどうかと無関係に要る（→ `docs/spec/quake-spec.md` §6.3
   * 「据え置いた電文は、音・読み上げ・タイトル・タブ移動も起こさない」）。
   *
