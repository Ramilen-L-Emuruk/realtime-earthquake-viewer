...
export interface EarthquakePoint {
   ...
   *
   * 印そのものは名前から外して持つ（`stripNonJmaMark`）。座標をはじめ、**印の無い名前を
   * 鍵にしている先がいくつもある**ため。**一覧は写さない** —— 単一情報源は
   * `docs/spec/quake-spec.md` §8「気象庁以外が運用する観測点」の表で、こちらへ書き写すと
   * 経路を足し引きしたときに片方だけ古くなる。
   *
   * **外すだけで事実を捨てないよう**、印の有無をここへ移し、**表示するときに名前へ戻す**
   * （`withNonJmaMark`）。
   */
  nonJma?: boolean
  /**
...
export interface JMAQuake {
   ...
   *
   * 長周期地震動の同じ枠（{@link JMALpgm.varCommentText}）と揃えた。実電文で入るのは
   * 観測点名の `＊` を説明する定型文（コード 0262）で、**震度を伝える電文のほぼ全てに
   * 入る**。アプリは印を名前へ戻して出しているので、この説明もそのまま出す
   * （→ {@link EarthquakePoint.nonJma}）。
   */
...
export interface EEWAlert {
  ...
  earthquake: {
    /**
     * 地震発生時刻（`Earthquake/OriginTime`）。**画面（`RealtimeTab`。秒まで出す）と
     * 共有カードに出すのはこちら。**
     *
     * 地震情報・津波・長周期は発現時刻を出すが、揃えているのは値ではなく**その種別で気象庁が
     * 地震の時刻として示すものに従う**という規則で、緊急地震速報ではそれが発生時刻になる。
     *
     * **表示専用ではない。** 発生時刻からの経過で決まる値（予報円・区域への S 波到達・自動解除・
     * 別地点発報の距離閾値）と、どれを「最新」とみなすかの並び順、リプレイの取得候補の絞り込みも
     * 同じ値を読む。
     * **読む場所の一覧・据え置きの根拠・実測は `docs/spec/eew-spec.md` §3** —— 値の意味を
     * 変えるときはそちらを見ること（**`grep` だけでは数え直せない** —— 別のオブジェクトへ写して
     * 運ぶ経路がある。ここへ一覧を写すと片方だけ古くなる）。
     */
    originTime: string
    /**
     * 地震発現時刻（`Earthquake/ArrivalTime`。観測点が地震を検知した時刻）。
     * 発生時刻（`originTime`）と別物。
     *
     * **読んで持つだけで、画面・読み上げ・共有カードのどこにも出していない。** 1 日分の実測では
     * 地震ごとに 1 通も動かず `EventID` と全通一致していた（母数と限界は下記リンク先）。中身は
     * 観測網の配置で決まる値で、**「仕様上不変」と確かめたわけではない**
     * （据え置きの根拠は → `docs/spec/eew-spec.md` §3「地震の時刻は発生時刻を出す」）。
     *
     * **出すことにするなら空文字を先に弾くこと** —— Yahoo hypoInfo 経路（standard 版）は
     * この値を配信しないため `services/kyoshin.ts` が空文字を入れる。
     */
    arrivalTime: string
    condition: string
    hypocenter: Hypocenter
...
/**
 * `onLiveEvent` が電文と一緒に運ぶ、**受信側で下した判断**。
 *
 * **電文の型（{@link LiveEvent}）へ混ぜない。** あちらは画面に載る状態そのもので、
 * 「このアプリがどう扱うと決めたか」を持たせる場所ではない（電文を組み立てる側・
 * 収録データ・リプレイの復元がすべてこの印を埋める羽目になる）。
 */
export interface LiveEventMeta {
  /**
   * その地震情報を**地震カードが採らない**（据え置く）か。
   *
   * 真なら通知音・読み上げ・ウィンドウタイトル・自動タブ切替・カードの選択・震度分布モードの
   * クローズを起こさない。**「この報は見た」の記録だけは従来どおり行う** —— 続報判定に要る、
   * カードが内容を採ったかどうかと無関係の処理（→ `docs/spec/quake-spec.md` §6.3
   * 「据え置いた電文は、音・読み上げ・タイトル・タブ移動・選択・分布モードのクローズも
   * 起こさない」）。
   *
   * **取消の報には立たない。** あれは内容の更新ではなくカードを消す操作で、据え置き判定の
   * 対象外（受信側が `!event.cancelled` で先に外す）。
   */
  quakeHeldBack?: boolean
}
...
/**
 * 推計震度分布図作図用データ（IXAC41・IXAC40）。
 *
 * 観測した震度と地盤増幅度から、震度計の無い場所の震度も推計した**面的な分布**。
 * 推計震度4以上の範囲を示し、最大震度5弱以上を観測した地震について、地震発生から概ね
 * 15 分後に発表される。
 *
 * **2 種別が同じ型に入る。** IXAC41 は 250m メッシュ（1/4 地域メッシュ）で、緊急地震速報の
 * 震度予測技術を考慮した手法で作られる。IXAC40 はその前身で 1km メッシュ（3 次メッシュ）。
 * 気象庁は 2026-02-02 に IXAC40 の配信を終了したので、**ライブで届くのは IXAC41 だけ**
 * ——IXAC40 は IXAC41 提供開始前の地震を再生したときにだけ現れる
 * （→ `data-sources-spec.md` §2「扱う電文種別」）。
 * **セルの寸法が違うので、描く側は {@link cellLatDeg} を読むこと。**
 *
 * **アプリが自前で描いている震度の面（`QuakeIntensitySurfaceGL`）の公式版**にあたる。あちらは
 * 観測点の値を逆距離加重で補間しただけだが、こちらは地盤増幅度と（IXAC41 では）緊急地震速報の
 * 震度予測技術まで織り込んだ気象庁の推計。
 *
 * **この電文だけ BUFR（二進形式）で届く。** DMDATA は他の種別と違って JSON 変換版を配らない。
 * 大きい電文は分割配信され、受け側で結合してから読む（→ `bufrTelegramAssembly.ts`。
 * **分割の符号の体系は 2 種別で違う**）。
 *
...
export interface JMAEstimatedIntensity {
  ...
  si: Uint8Array
  /**
   * セル 1 つの緯度方向の寸法（度）。**電文の種別で変わる** —— IXAC41 は 1/4 地域メッシュ
   * （約 250m）、IXAC40 は 3 次メッシュ（約 1km）。
   *
   * **モジュール定数から引かないこと。** テクスチャの寸法（`estimatedIntensityRaster.ts`）と
   * 矩形の閉じ方（{@link bounds}）がこの値で決まるので、定数で持つと片方の種別で必ずずれる。
   * ずれても例外は出ず、**セルの間に隙間が空くか重なるだけ**なので画面では気づけない。
   */
  cellLatDeg: number
  /** 同・経度方向。IXAC41 は 1/320 度、IXAC40 は 1/80 度 */
  cellLonDeg: number
  /**
