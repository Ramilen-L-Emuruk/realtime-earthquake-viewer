...
| `KyoshinSubThreshold` の対象範囲（index 1〜6）・慢性ノイズ床フィルタ（**床は観測点と同じ並びの配列で渡す**。座標をキーにすると同一座標の 2 つ目以降が別センサーの床で判定される／位置で使う前に、その床がこの観測点配列に対して計算されたものかを参照で照合する＝床は 1 レンダー遅れて届く／**検知結果を位置で消費する経路を新しく足すときは、同じ照合が要らないかを確かめる**） | [`docs/spec/kyoshin-detection-spec.md`](docs/spec/kyoshin-detection-spec.md)・[`docs/spec/data-sources-spec.md`](docs/spec/data-sources-spec.md) §4「取得と再生の分離」 |
