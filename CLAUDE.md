...
| 地図の傾き（pitch）・回転（bearing）が常態であることに依存する箇所（**予報円は地面に貼る**＝画面座標で半径を測らない／**カメラフィットは `bearing` を渡す**＝渡さないと回転が無断リセットされる／**収まり判定は目標の四隅を `map.project()` で画面座標へ投影して見る**＝地理座標の矩形どうしで比べると回転時にはみ出しを見逃す／**ラベルの退避は符号で判定**＝`room` は真北・真南固定の生成データなので絶対値では読み違える／**操作検知に `rotatestart`・`pitchstart`**）。`maxPitch` の値の意味。**新たに傾き・回転を前提にする箇所を足したらここへ追記する** | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §6「地図の傾きと回転」 |
