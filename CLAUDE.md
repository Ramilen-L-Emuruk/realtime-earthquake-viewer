...
| レイヤーの表示境界（`minzoom`/`maxzoom`）をカメラの着地段（0.5 刻み）に乗せないこと（`flyTo` は開始と目標が同じズームでも移動中に必ず下回るため、同値だとその段へ着地する操作のたびにレイヤーがまるごと消え、着地で戻る。落ち込み幅は移動距離に比例する） | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §6「閾値を触るときに併せて見直すもの」 |
