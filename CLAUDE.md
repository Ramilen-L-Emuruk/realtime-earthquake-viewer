...
| 深さを持つ点の描画（震源・将来の震源カタログ）。**MapLibre の far は地下を含まない**（傾き 0 では地表の 1.01 倍まで）ので、シェーダーの中で z を差し替える／**表示用と判定用に同じ差し替えを入れる**／クリック判定はカラーピッキングで、**未解決（`'pending'`）を「何も無い」と区別する**（タッチ操作は `mousemove` を伴わないため）／柄と震央の印は**柄の画面上の長さ**で隠す（傾きだけで判定しない） | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §16「深さを持つ点」 |
