...
| 深さを持つ点の描画（震源・将来の震源カタログ）。**地下の点は 2 つの理由で消える**（平面は far が地下を含まない／球は地表のタイルが深度を書いて地面に隠される）ので、クリップ空間の z を**手前の薄い帯**へ自前で写す／帯の下限は丸めない（near より手前は従来どおり切る）／**球では地球の中心を越えさせない**（越えると点が反対側へ写る。深さ 700km × 強調 10 倍で届く）／**表示用と判定用に同じ計算を入れる**／クリック判定はカラーピッキングで、**未解決（`'pending'`）を「何も無い」と区別する**（タッチ操作は `mousemove` を伴わないため）／柄と震央の印は**柄の画面上の長さ**で隠す（傾きだけで判定しない）／**球の裏側へ回った点は透けたまま残す**（`projectTileFor3D` が水平線のクリッピングを掛けず、z を帯へ書き換えるのも重ねて効く。遠地地震の震源は日本と同じ視野に入らないため、そのままにしている。裏を消す方へ転じるなら**可視面の判定は震央の位置で行う**——点の座標へ当てると手前にある深い震源まで消える） | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §16「深さを持つ点」 |
