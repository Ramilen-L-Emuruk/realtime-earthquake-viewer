...
> main を書き換える操作はすべて下記「リリース」に集約し、専任のセッションが一括で行う。
> 実装セッションが main を触ってはならない（理由は「リリース」節の「原則」を参照）。
...
- **リリース専用の新規セッション**を立て、`/release` と入力するか「リリースして」と指示してスキルを起動する。実装作業と同じセッションでは行わない。
- スキルは起動時に `ready/*` を探し、あればリリース手順へ進む。無ければ**ブランチ監視モード**に入り、`ready/*` が現れた時点でユーザーに確認する。
- 監視中に検知しても、**ユーザーの承認なしに main を触ることはない**。

### 原則

- **実装セッションはリリース操作を一切行わない。** マージ・バージョン更新・プッシュ・マージ済みブランチの削除は、リリースを指示されたセッションだけが行う（背景は冒頭の 2026-08-15 の注記を参照）。
  - そもそもワークツリー内から main を正しく操作できない（理由はスキル側の「着手前チェック」に記載）。
- **1 回のリリースで複数ブランチをまとめて出してよい。** バージョンはリリース 1 回につき 1 回だけ上げる（機能 1 本ごとではない）。
- **リリースセッションは同時に 1 つだけ。** 並行して走らせると、両方が同じバージョン番号・同じタグを採番して push が衝突する。
