...
| 通知音の音量の決め方（**種別ごとの個別の値を書かない**・`BASE_GAIN`（系統）×`SEVERITY`（深刻度・3 dB 刻みの 5 段）の積だけで決める・震度更新音は `BEEP_SEVERITY` が段を補間し `BEEP_PATTERNS` は音量を持たない・S 波カウントダウンも同じ設計に載っている・**同じ値でも音数と重なりで音圧は変わる**ので、揃えるために片方の値だけ動かさない） | [`docs/spec/audio-tts-spec.md`](docs/spec/audio-tts-spec.md) §2「種別ごとの音量は「系統 × 深刻度」で決める」 |
