...
| 通知音の系統の分け方（5 つのプリミティブ＝`pianoNote` / `darkPiano` / `darkAlarm` / `sweep` / `marimba` のどれかに属させる・**残響は全系統で持たない**（連続して鳴る音で尾が重なり濁るため廃した。豊かにする目的で戻さない）・フィルタを使うのは `marimba` だけ・減衰の終端は `decayTone` に集約し種別ごとに分岐させない・アタックに広帯域ノイズを戻さない・厚みはユニゾン（音階を持つ音）／非整数倍音（情報系）／ステレオ（警報系）／木琴の共鳴比（検知系）で作り、**どれをどこに効かせるかを混ぜない**） | [`docs/spec/audio-tts-spec.md`](docs/spec/audio-tts-spec.md) §2「系統の分け方 — 5 つのプリミティブ」 |
...
| 長期震源カタログの読み取り（**仕様書の「96 バイト固定長」を鵜呑みにしない**＝実ファイルは改行区切り／**数値欄の小数部の桁数が年代で変わる**＝1961〜1993 年は 1 桁少なく、素朴に読むと緯度が 78km ずれる（1983・1990・1993 年は全件が該当）／仕様外の深さは値を推測せず捨てる（`   0 ` は 0〜9 すべてが現れるので「0km の別表記」ではない）／採るのはレコード種別 `J` だけ／時刻は JST なので `Date.UTC` で組み立ててから 9 時間引く（CI は UTC で回る）／**取得経路ごとに既知の地震と照合する**＝7 経路・7 地震。範囲を絞ったときは照合していない経路を名指しで出す／**検知した異常は必ず exit 1 に繋げる**＝`console.warn` で終わらせると CI は終了コードしか見ないので壊れたデータが本番へ出る／**M 欄だけが壊れると「読めない行」は増えず件数 0 という形でしか現れない**ので、そこも失敗条件に入れる／格納単位・時刻の起点・データの終端・確からしさは**年ファイル自身に持たせる**（索引だけに置くと、読み損ねたとき桁がずれた値や速報値を確定値として黙って使う）／**索引は出力ディレクトリの実態から作る**（今回生成した年だけで書くと、範囲を絞った再生成で他の年が索引から消える）／**M2.0 の完全性は 1997 年以降だけ**なので、件数を数える機能は `completeMinMagnitude()` で閾値を上げる／直近 30 日のヒートマップとは母集団が 10 倍以上違うので統計に混ぜない） | [`docs/spec/data-sources-spec.md`](docs/spec/data-sources-spec.md) §6「長期震源カタログ」 |
