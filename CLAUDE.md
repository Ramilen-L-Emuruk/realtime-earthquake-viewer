...
| 区域を読めなかった結果を「正式解除」と取り違えないこと（**XML 経路限定**。区域を**有効／解除済み／解除と言えない**の 3 つに分け、**解除と断定してよいのは「解除と言えない」区域が 1 つも無いときだけ**／解除済みの判定は解除コード（00/50/60）が読めたかどうかで、**名前の可否を問わない**／**他の区域に解除コードがあることは、そちらについて何も保証しない**——ここを「1 件でもあれば通す」にすると、現役の等級のまま名前だけ壊れた区域を巻き込んで解除を発表する／区域が電文から消えた形は有効 0・「解除と言えない」0 で従来どおり解除／JSON 経路は名前が空でも区域を積むため 0 件にならない） | [`docs/spec/tsunami-spec.md`](docs/spec/tsunami-spec.md) §4「DMDATA XML 履歴」 |
