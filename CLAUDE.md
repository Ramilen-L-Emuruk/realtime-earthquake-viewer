...
| 深さを持つ点の描画（震源・将来の震源カタログ）。**地下の点は 2 つの理由で消える**（平面は far が地下を含まない／球は地表のタイルが深度を書いて地面に隠される）ので、クリップ空間の z を**手前の薄い帯**へ自前で写す／帯の下限は丸めない（near より手前は従来どおり切る）／**球では地球の中心を越えさせない**（越えると点が反対側へ写る。深さ 700km × 強調 10 倍で届く）／**表示用と判定用に同じ計算を入れる**／クリック判定はカラーピッキングで、**未解決（`'pending'`）を「何も無い」と区別する**（タッチ操作は `mousemove` を伴わないため）／柄と震央の印は**柄の画面上の長さ**で隠す（傾きだけで判定しない） | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §16「深さを持つ点」 |
...
| 地図の投影が球と平面を行き来すること（寄ると MapLibre が自動で Mercator へ切り替える・境目は zoom 11 と 12 のあいだ）。**カスタムレイヤーは行列 1 本では描けない**——MapLibre が配る投影ごとの GLSL 断片を頂点シェーダーへ貼り、**プログラムを投影ごとに持つ**／属性の番号はリンク前に固定して VAO を使い回す／**リンクに失敗したプログラムを返さない**（未リンクへの `useProgram` は例外を投げず別レイヤーのシェーダーが残る）／`projectTileFor3D` の elevation は**球はメートル・平面は Mercator の z** で単位が違う／**`map.project()` は地球の裏側にも画面内の座標を返す**ので、見えているかを画面座標で判定する箇所は投影して戻す往復で弾く | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §6「地図の投影」 |
