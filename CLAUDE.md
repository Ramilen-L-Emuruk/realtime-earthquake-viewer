...
| 揺れフォーカス（揺れが強まった点・別地点へ一時的に寄る）の発火条件と寄り先（**寄るのは最大震度を記録した観測点でメンバー重心ではない**・音と同じ判定で要求を出す・likely では寄せない・EEW とユーザー操作中と古い要求は見送る・見送っても連番は消費する・戻す経路は成長フォローに任せる・消費済み連番は常時マウント側で覚える） | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §6 |
