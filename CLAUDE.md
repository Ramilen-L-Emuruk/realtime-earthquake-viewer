...
| 続報で上書きするとき、incoming に無い項目を既存カードから補うこと（**補うのは「その電文種別が構造的に持たないフィールド」だけ**＝震度速報に震源要素は無い／震源情報に震度は無い。**震度速報が自前で持つ津波区分・付加文は補わない**——電文が主張する値なので従う。**同じフィールドの中で値が消えた場合も補わない**——保持すると気象庁が取り下げた値を出し続ける。震源側は取消表示中のカードからも補わない（震度側はこの判定を持たない）・判定は `isHypocenterPending` を使い震源名の空判定を書き写さない・**テストの震度速報は実電文どおりのヘルパーで作る**（`makeQuake({ type: '震度速報' })` は震源を持たせてしまい、震源が消える穴を検出できない）） | [`docs/spec/quake-spec.md`](docs/spec/quake-spec.md) §6.4 |
