...
11. **リリース待ちへの登録**（ブランチ名の接頭辞を `ready/` に付け替える。**省略するとそのブランチは永久にリリースされない**）
    ...
    - **確認の出し方**は状況で決める
      ...
      | 発言から一意に決まらないが、**作業が全て完了したと判断できる**（ユーザーが完了を明言した、または続きの作業が無いことが双方で明らかな場合） | 4 択を出す |
...
| ユーザーの言葉 | 意味 |
|---|---|
| 「コミットして」 | 手順 9〜10 を実行する。**作業が全て完了したと判断できない限り、手順 11（`ready/` 改名）の話は持ち出さない**（続きの作業がある間はコミットのたびに聞き直さない。完了と判断できた時点でのみ 4 択を出す） |
