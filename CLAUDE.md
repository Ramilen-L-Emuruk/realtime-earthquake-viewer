...
| 長期震源カタログの点群（震源カタログタブ）。**点の入口は 2 つ**（オブジェクトの配列＝震源マーカー／列指向の型付き配列＝カタログ）で、**バッファへの書き込みは 1 箇所へ集約する**（`writePointInto`。属性を足したとき片方だけ直すと静かにずれるので、単体テストで `POINT_LAYOUT` と突き合わせる）／列指向では柄・補助点・点滅を持てない／**カタログは進行中の地震より背面**／**期間を変えたらマグニチュードの下限を完全性へ合わせる**（合わせないと古い時代が「地震が少なかった」ように見える。手で下げるのは妨げず注意書きを出す）／深さの色は**平方根で浅い側へ寄せる**／発生年の色の分母は**絞り込みに残った点で測る**／色の位置の丸めは**真のときだけ通す形**で書く（`t <= 0 ? 0 : ...` は NaN が素通りして色が NaN になる）／**完全性の判定は期間の最も古い年で行う**（`oldestYearOf`。「開始」と「終了」は別々に選べるので `fromYear` を直に見ると、読んでいない年を基準に「網羅している」と答える。セレクトは押し合わせて逆転そのものを作らせる前に潰す）／**スライダーは止まってから作り直す**（全期間で組み立て 16ms ＋ 転送 52ms。つまみとラベルは待たせない）／**遅らせる先は 2 つ**（点群の作り直しと**年ファイルの取得**。後者を繋いだままだと、つまみを引きずった途中の全ての年で取得が走り、発行済みの取得は打ち切られないのでドラッグ 1 回で全ファイルを取りに行く）／**絞り込みの端は「制限なし」として扱う**（`effectiveMaxDepthKm`。端の値で切ると、より深い地震が入ったとき黙って消える。見出しも「700km 以浅」と書かない——それより深いものは見られないと読める）／**絞り込みで選べる範囲は色が飽和する範囲と別に持つ**（`MAGNITUDE_FILTER_RANGE` と `MAGNITUDE_RAMP_RANGE`。借りると色の調整で絞り込みの届く範囲まで動く）／**強調表示の倍率は 0 まで落とさない**（傾きだけで決めると収録の下限 M2.0 で 0 倍になり、最も件数の多い地震が消える）／**絞り込みは 3 つとも二つまみのスライダー 1 本**（期間・マグニチュード・深さ。`RangeSlider`。`range` を 2 本重ね、本物の range のままにしてキーボード操作と読み上げを残す・重なったら下端を上に置く・無効時はつまみを薄くする）／説明文はホバーで出す（設定タブと同じ `DescriptionTip`。**範囲の数値だけは常に出す**——つまみを見ただけでは値が読めない）／**隠すときは点を持たせたまま止める**（`setVisible`。空を渡すと戻るたびに詰め直す）／**1 年の失敗で期間全体を捨てない**（`Promise.allSettled`・欠けた年は名指しで伝える）／**取得に失敗したら前の期間の点を残さない**（残すと期間の表示と中身が食い違ったまま固定され、しかも読み込みが終わったように見える）／ポップアップの深さに `formatDepth` を使わない（整数の深さ向けで丸めを持たず、単精度の誤差が露出する） | [`docs/spec/map-rendering-spec.md`](docs/spec/map-rendering-spec.md) §16「長期震源カタログの点群」 |
