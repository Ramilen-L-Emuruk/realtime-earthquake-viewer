...
| 通知音の長さと読み上げ遅延の連動（音を作り変えたら `TTS_DELAY_MS` かテーブル外の個別指定を必ず見直す。**残響は全系統で廃したので、音の終わり（ピーク比 −40dB）がそのまま見積もりの起点になる**） | [`docs/spec/audio-tts-spec.md`](docs/spec/audio-tts-spec.md) §6 |
