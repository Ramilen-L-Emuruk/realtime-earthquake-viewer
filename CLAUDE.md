...
| `npm run build-hypocenter-catalog` | 長期震源カタログ（`public/data/hypocenter/`）の再生成。気象庁の年別 ZIP を読み、既定で 2009〜2023 年・M2.0 以上を年ごとの JSON にする（`--from` / `--to` / `--min-magnitude` で変更可）。**API キー不要**。**2011 年を範囲に含めて生成すると**東北地方太平洋沖地震の本震を既知の値と照合し、合わなければ止まる（カラム位置のずれは型でも実行時エラーでも捕まらないため）。範囲を絞って含めなかった場合は照合せず、その旨を出力に残す。詳細は [`docs/spec/data-sources-spec.md`](docs/spec/data-sources-spec.md) §6「長期震源カタログ」 |
...
| 長期震源カタログの読み取り（**仕様書の「96 バイト固定長」を鵜呑みにしない**＝実ファイルは改行区切りで、バイトで切ると 1 バイトずつずれる／深さは**2 つの書き方**があり後ろ 2 桁が空白かで見分ける／稀に仕様外の深さがあり値を推測せず捨てる（**M で絞る前の `J` レコード**に対する割合が 0.02% を超えたら警告）／採るのはレコード種別 `J` だけ／時刻は JST なので `Date.UTC` で組み立ててから 9 時間引く（CI は UTC で回る）／**2011 年を含めて生成すれば既知の地震と照合する**（含めなければ照合されない。範囲を絞って再生成したものを「照合済み」と思わないこと）／格納単位と時刻の起点は**年ファイル自身に持たせる**（索引だけに置くと、読み損ねたとき桁がずれた値を黙って使う）／**索引は出力ディレクトリの実態から作る**（今回生成した年だけで書くと、範囲を絞った再生成で他の年が索引から消える。ファイルが在って索引に無い年は件数を読んで拾う）／直近 30 日のヒートマップとは母集団が違うので統計に混ぜない） | [`docs/spec/data-sources-spec.md`](docs/spec/data-sources-spec.md) §6「長期震源カタログ」 |
