...
| 項目 | 単一情報源となる仕様書 |
|---|---|
| 共有カードの撮影と出典（WebGL のキャンバスは `render` の同期実行を抜けると空になるので**写し取りは同期の窓で行う**・`preserveDrawingBuffer` は初期化オプションなので実行時に切り替えられない・**撮影はカメラに触らない**（動かす経路を足すなら `camera.ts` の fit 系を通し `beginProgrammaticFlight` の印を渡す。落とすと地図の自動追従が 30 秒止まる）・**撮影中は地図コンテナの親を切り抜く**（出力寸法は画面より大きく、隣のパネルへはみ出す）・**後始末は 1 つずつ囲う**（静止画の除去に届かないと、地図が覆われて変化が画面に現れなくなる）・**出典は描いているものだけを挙げる**（プレート境界の PB2002 は帰属表示が利用条件・レイヤーを足したら `ATTRIBUTION_SOURCES` も見直す）・緊急地震速報の注意文の要否は**モードではなく発報の有無**で決める（予報円は全モードで描かれる）・**仮定震源要素では規模を出さない**（画像は訂正できない）・津波の観測棒は DOM マーカーなので 2D で描き直す） | [`docs/spec/share-card-spec.md`](docs/spec/share-card-spec.md) |
